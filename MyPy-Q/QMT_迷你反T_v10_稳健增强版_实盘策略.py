...
def _print_startup(ContextInfo, base_shares, base_can_use, base_cost):
    ...
    _log(f'{"="*55}')
    _log(f'  {STOCK_NAME} v10.0 稳健增强版 — 已加载')
    _log(f'  交易日: {today}')
...
def _submit_buyback(ContextInfo, price, reason=''):
    """买回下单 — 先检查资金"""
    st = ContextInfo.st
    ...
    _submit_order(ContextInfo, TRADE_LOT_SIZE, price, f'买回({reason})')
...
def stop(ContextInfo):
    ...
    if st:
        ...
        _log(f'\n{"="*55}')
        _log(f'  {STOCK_NAME} v10.0 停止 | 累计 {st.get("total_t_days", 0)}天 | 毛利≈¥{st.get("total_pnl", 0):,.0f}')
        if errors > 0:
