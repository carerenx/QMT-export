...
"""
================================================================================
 MiniQMT 适配版 — QMT 迷你反T v12 锁仓增强策略
================================================================================
...
# ============================================================================
# ★ v12: 锁仓参数 — 盘面过强时禁止卖出, 防止卖飞
# ============================================================================
# 当以下条件同时满足时, 锁定仓位不卖:
#   (1) 现价 > 开盘价 × (1 + LOCK_PRICE_RATIO)    — 价格远超开盘
...
class StrategyRunner:
    ...
    def _daily_init(self):
        ...
        # 正T(先买后卖): 需要 (1)可用资金>=1手 AND (2)有T+0可卖持仓>=1手
        #   T+1规则: 今日买入的股票今日不能卖, 卖必须用原有可用持仓
        can_sell_later = base_can_use >= TRADE_LOT_SIZE
        do_long = avail_cash >= lot_cost and can_sell_later
        long_reason = ''
        if not do_long:
            reasons = []
            if avail_cash < lot_cost:
                reasons.append('资金不足(需Y{:,.0f}>Y{:,.0f})'.format(lot_cost, avail_cash))
            if not can_sell_later:
                reasons.append('T+1: 无可卖持仓(可用{}股<1手)'.format(base_can_use))
        ...
        if st.get('do_short', False):
            ...
            if price >= trigger:
                ...
                if st.get('locked', False):
                    _log('[反T锁定] 盘面强势({}), 跳过卖出 触发Y{:.2f} 现价Y{:.2f}'.format(
                        st.get('lock_reason', ''), trigger, price))
                    return
