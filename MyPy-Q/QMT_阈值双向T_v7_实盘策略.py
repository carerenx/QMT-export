...
================================================================================
 ...
 【v7.0 — 心跳优化 + 所有v6防护保留】
  ...
  v6已知问题:
    - _reset_daily_state 被反复调用(根因不明, 可能与QMT定时器调度有关)
    - sell_side_done 被反复清空 → 同一日内连续卖出十几次
    - 心跳显示开盘价(¥310.56)而非实时价, 用户看不到当前行情
    - 心跳在闭市和SOLD状态过于频繁

  v7修复:
    - ★ 日期锁 _guard_date: _reset_daily_state 同一天绝不允许调用第二次
    - ★ 卖出加固: _handle_idle 先标 sell_side_done 再下单(防重入)
    - ★ 心跳用状态机已获取的price: 不再二次调 get_full_tick(防洪水)
    - ★ 心跳仅在活跃交易状态输出(DONE/FORCED时静默)
    - ★ 状态切换日志去重
    - ★ 启动信息每天仅一次 (_startup_guard)
...
# ── ★ 核心参数(用户输入) ──
SELL_TRIGGER_PCT = 0.03            # 卖出触发: 涨超开盘价4%
BUY_TRIGGER_PCT  = 0.01            # 买入触发: 跌超开盘价3%
...
def _print_startup(ContextInfo):
    ...
    _log(f'{"="*55}')
    _log(f'  {STOCK_NAME} 阈值双向T v7.0 — 心跳优化版')
    _log(f'  交易日: {today}')
