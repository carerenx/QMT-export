    ...
    try:
        while True:
            ...
            for code in STOCK_LIST:
                ...
                chg = q["change_pct"]
                arrow = "+" if chg > 0 else ""
                # 计算相对强弱: 个股涨跌幅 / 上证指数涨跌幅
                # 指数涨跌 < 0.05% 时视为横盘，不计算强弱（避免分母过小导致数值失真）
                if raw_code(code) != "000001" and abs(idx_chg) > 0.05:
                    strength = max(-10, min(10, chg / idx_chg))
                    print(f"{arrow}{q['price']:.2f}  ({chg:+.2f}% 强{strength:+.1f})", end="  ")
                else:
