# -*- coding: gbk -*-
"""MiniQMT CSI 500 MA screener v8 with Chinese stock names."""
...
def load_selected_names(xtdata, selected):
    names = {}
    for _, code, _ in selected:
        try:
            detail = xtdata.get_instrument_detail(code) or {}
            name = str(detail.get('InstrumentName') or '-').strip() or '-'
        except Exception:
            name = '-'
        names[code] = name
    return names
...
def format_result_line(code, name, result):
    return '%-12s %s %10.2f %10.2f %10.2f %9.2f%%' % (
        code,
...
def run_screen(xtdata, today=None):
    ...
    print('CSI 500: current price < MA5 by {:.2f}% and current price > MA20'.format(
        MA5_BELOW_PERCENT))
    print('code         name              price        MA5       MA20    vs_MA5')
...
def main():
    ...
    if not connector.connect_data():
        return 1
    try:
        print('[START] CSI 500 MA5/MA20 screener v8 (Chinese stock names)')
