# -*- coding: utf-8 -*-
"""v0561 CoreT — the 反T / 正T mechanism and nothing else.

Derived from DayT_v056_nomom_SimplifiedReversal.py by deleting every layer that
only exists to serve a *continuous* account:

  removed  checkpoint read/write, manual reconciliation, runtime watchdog,
           connection monitor, MA report, risk budget / resume timers,
           directional admission, cycle ownership ledger, quantile trend
           regime, ATR re-entry, intraday strength, limit-up guard
  removed  the forward-T stop-loss (cfg.STOP_LOSS_PCT) and the end-of-day
           force close (cfg.ENABLE_FORCE_CLOSE)

Intended use is DAILY RESET: each session starts from a fixed cash balance and
base position with fresh state.  A T leg that cannot close is simply left open
and discarded with the session; nothing is carried across days and nothing is
reconciled on restart.

Kept verbatim from v056: compute_signal (ATR / open-anchored sell trigger /
do_short gate / range cap), confirmed_short_reversal, the REV-T ladder
IDLE→SPIKING→SOLD→DIPPING and the FWD-T ladder IDLE→BT_DIPPING→BT_BOUGHT→
BT_SPIKING, whole-lot sizing via calculate_t_shares, and ExecutionBook
accounting.
"""
...
# Permit the documented ``python Stragety/.../DayT_v0561_CoreT.py`` command.
...
class StrategyRunner:
    ...
    def _recalculate_next_t_triggers(self, completed_by):
        """Daily-reset build: the day's signal stays the day's trigger."""
        st = self.st
        st['next_t_cycle'] = st.get('next_t_cycle', 0) + 1
        self._log('[CYCLE-{} #{}] closed; thresholds stay at the daily signal'.format(
            completed_by, st['next_t_cycle']))
    ...
    def run(self):
        ...
        try:
            ...
            self._log('[STOP] {} v0561 cum {} days gross~Y{:,.0f}'.format(
...
class PortfolioRunner:
    ...
    def run(self):
        ...
        if not self.conn.connect_trade():
            ...
            return
        _log('[PORTFOLIO-START] v0561 CoreT; mode={}'.format(
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='v0561 CoreT: forward-T / reverse-T only, daily reset, no stop-loss')
    parser.add_argument('--mode', default='signal', choices=['signal', 'live'])
    args = parser.parse_args()
    logger = FileLogger('portfolio', version='v0561')
    set_logger(logger)
    try:
        if args.mode == 'live':
            print('LIVE: apply v0561 CoreT to {}. Account: {}'.format(
