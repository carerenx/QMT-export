# -*- coding: utf-8 -*-
"""
v056 SimplifiedReversal — v56 with one lane and without ladder, dynamic rebound reference, or maximum-holding buyback.
...
LONG_RESEARCH_DISABLED = True  # v056：正T消融为负，冻结该通道直至独立样本验证通过。
OVERNIGHT_ENABLED = True
CYCLE_EXPOSURE_FRACTION = 0.50  # 单个未平周期数量/原始底仓。
...
STATE_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'state', 'v056_{}.json'.format(cfg.ACCOUNT))
...
class ExecutionRunner:
    """MiniQMT v056 — simplified confirmed reversals with a protected core position."""
    ...
    def run(self):
        ...
        try:
            ...
            self._log('[STOP] {} v056 cum {} days gross~Y{:,.0f}'.format(
...
class StrategyRunner(ExecutionRunner):
    """Research runner: one independently owned cycle per stock."""
    ...
    def _new_leg_block_reason(self):
        return self.paused_reason or ('runner already owns an unfinished cycle'
    ...
    def _new_t_shares(self, price, side):
        ...
        if self.baseline_shares is None:
            return 0
        if self.cycle and self.cycle.quantity:
            return 0
    ...
    def _submit_order(self, shares, price, label, style='COMPETE'):
        ...
        if opening:
            ...
            if count >= cfg.MAX_DAILY_TRADES:
                ...
                return 'SKIP', 0
            if label == 'FWD-T buy' and DIRECTIONAL_ENABLED and not self.admission.allowed:
...
class PortfolioRunner(ExecutionPortfolio):
    """One serialized per-stock runner. Live entry remains disabled during research."""
...
def main():
    parser = argparse.ArgumentParser(description='v056: simplified confirmed reversals and protected core position')
    ...
    logger = FileLogger('portfolio', version='v056')
    set_logger(logger)
    try:
