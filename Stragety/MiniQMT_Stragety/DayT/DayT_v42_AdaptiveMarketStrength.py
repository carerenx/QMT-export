    ...
    def _print_daily_brief(self, signal):
        ...
        if signal.get('adaptive_linear_active'):
            _log('[ADAPTIVE-LINEAR] score {score:+.4f} | bear<={bear:+.4f} | bull>={bull:+.4f} | '
                 ...
                 'mean {mean:.3f} | attainable {attainable:.3f} | '
                 'trigger ATR-units {units:.3f}'.format(
                     ...
                     mean=adaptive.get('predicted_units', 0.0),
                     attainable=adaptive.get('attainable_units', 0.0),
                     units=adaptive.get('trigger_units', 0.0)))
            contributions = adaptive.get('trigger_contributions', {})
            contribution_text = ' '.join(
                '{}{:+.3f}'.format(name[:3], value)
                for name, value in contributions.items())
            if adaptive.get('trigger_mode') == 'bull_trend_blend':
                _log('[ADAPTIVE-UNIT-FORMULA] bull-blend units=low-tail '
                     '{low_tail:.3f} + {blend:.3f}*(mean {mean:.3f} '
                     '- low-tail {low_tail:.3f}) = {units:.3f}; mean=base '
                     '{base:.3f} + {terms}'.format(
                         low_tail=adaptive.get('lower_tail_units', 0.0),
                         blend=adaptive.get('bull_blend', 0.0),
                         mean=adaptive.get('mean_units', 0.0),
                         units=adaptive.get('trigger_units', 0.0),
                         base=adaptive.get('trigger_intercept', 0.0),
                         terms=contribution_text or 'no-feature-contribution'))
            else:
                _log('[ADAPTIVE-UNIT-FORMULA] units=clip(mean {mean:.3f} '
                     '(base {base:.3f} + {terms}) + lower-residual '
                     '{strength:.2f}*{residual:+.3f}, [{lower:.3f}, '
                     '{upper:.3f}]) = {units:.3f} (tail {tail:.3f})'.format(
                         mean=adaptive.get('predicted_units', 0.0),
                         base=adaptive.get('trigger_intercept', 0.0),
                         terms=contribution_text or 'no-feature-contribution',
                         strength=adaptive.get('trigger_lower_bound_strength', 0.0),
                         residual=adaptive.get('trigger_residual_lower_quantile', 0.0),
                         lower=adaptive.get('trigger_lower_bound', 0.0),
                         upper=adaptive.get('trigger_upper_bound', 0.0),
                         units=adaptive.get('trigger_units', 0.0),
                         tail=adaptive.get('trigger_tail_rank', 0.0)))
