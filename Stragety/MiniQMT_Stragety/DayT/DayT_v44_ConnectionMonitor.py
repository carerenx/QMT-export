...
"""
================================================================================
 MiniQMT — QMT day trading v44：趋势分位数反T阈值 + 连接监控
================================================================================

 [v44 改动] (基于 v43 独立策略代码)
   - 以 v42 完整策略代码为基线独立构建；不导入或继承任何历史策略文件。
   - 用当前趋势在历史中的分位数，直接选择历史可达涨幅分位数。
   - 强趋势抬高反T启动价以保留趋势收益，弱趋势降低启动价以提高频率。

 [run mode]
 python "Stragety/MiniQMT_Stragety/DayT/DayT_v44_ConnectionMonitor.py" --mode signal
 python "Stragety/MiniQMT_Stragety/DayT/DayT_v44_ConnectionMonitor.py" --mode live
 python "Stragety/MiniQMT_Stragety/DayT/DayT_v44_ConnectionMonitor.py" --mode backtest

 python run_bigqmt.py --strategy Stragety/MiniQMT_Stragety/DayT/DayT_v44_ConnectionMonitor.py --mode live
================================================================================
"""
...
# Permit the documented ``python Stragety/.../DayT_v44...py`` command.
...
class StrategyRunner:
    """MiniQMT v44 — 趋势分位数反T与周期性连接监控。"""

    def __init__(self, dry_run=False):
        logger = get_logger()
        if logger is None:
            logger = FileLogger(STOCK_CODE, version='v44')
            set_logger(logger)
        self.version = 'v44'
    ...
    def _monitor_connections(self, now_ts):
        ...
        healthy, detail = probe_connections(
            self.conn, self.ctx, STOCK_QMT, check_trade=not self.dry_run)
        if not healthy:
            _log('[CONNECTION-ALERT] {}'.format(detail))
        elif self._connection_healthy is False:
            _log('[CONNECTION-RECOVERED] {}'.format(detail))
        elif self._connection_healthy is None:
            _log('[CONNECTION] {}'.format(detail))
    ...
    def _print_daily_brief(self, signal):
        ...
        if signal.get('quantile_trend_active'):
            _log('[QUANTILE-TREND] score {score:+.3f} | rank {rank:.3f} | '
            ...
            _log('[QUANTILE-FORMULA] units=Quantile(next-day open-to-high, '
    ...
    def run(self):
        ...
        try:
            ...
            _log('[STOP] {} v44 cum {} days gross~Y{:,.0f}'.format(STOCK_NAME, self.total_t_days, self.total_pnl))
            logger = get_logger()
...
def main():
    parser = argparse.ArgumentParser(description='MiniQMT v44 - trend quantile REV-T with connection monitor')
    ...
    logger = FileLogger(STOCK_CODE, version='v44'); set_logger(logger)
