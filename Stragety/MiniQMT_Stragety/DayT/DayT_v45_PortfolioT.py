...
"""
================================================================================
 MiniQMT — QMT day trading v45：账户全部持仓独立做T + 连接监控
================================================================================

 [v45 改动] (基于 v44 独立策略代码)
 ...
 [run mode]
 python "Stragety/MiniQMT_Stragety/DayT/DayT_v45_PortfolioT.py" --mode signal
 python "Stragety/MiniQMT_Stragety/DayT/DayT_v45_PortfolioT.py" --mode live
...
# Permit the documented ``python Stragety/.../DayT_v45...py`` command.
...
class StrategyRunner:
    ...
    def _monitor_connections(self, now_ts):
        ...
        if now_ts - self._last_connection_check < CONNECTION_CHECK_INTERVAL_SEC:
            return
        self._last_connection_check = now_ts
        healthy, detail = probe_connections(
            self.conn, self.ctx, self.stock_qmt, check_trade=True)
        if not healthy:
    ...
    def _daily_init(self):
        ...
        self._log('[DATA-DBG] last_complete={} hist_open[-3:]={} | tick_open={} | '
    ...
    def _recalculate_next_t_triggers(self, completed_by):
        ...
        try:
            ...
            self._log('[NEXT-T RECALC SKIP] {} done, latest tick unavailable: '
        ...
        if not st.get('intraday_avg_valid', False):
            self._log('[NEXT-T RECALC SKIP] {} done, current tick intraday '
        ...
        if not triggers:
            self._log('[NEXT-T RECALC SKIP] {} done, intraday average unavailable; '
        ...
        self._log('[NEXT-T RECALC #{}] {} done | avg Y{:.4f} | REV Y{:.2f}→Y{:.2f} '
    ...
    def _submit_order(self, shares, price, label, style='COMPETE'):
        ...
        order_shares(self.stock_qmt, signed, style, price, self.ctx, ACCOUNT)
    ...
    def _verify_trade(self, snap_before, label, trade_price, trade_shares):
        ...
        self._log('[FILL-{}] {} × {} sh | pos {}→{} | cash {:+,.0f}'.format(
            label, price_str, abs(trade_shares),
            snap_before['shares'], snap_after['shares'], d_cash))

        # 仅在异常时输出校验详情
        if status != 'OK':
            self._log('[VERIFY] {}: status {} | exp {:+d} act {:+d} | cashΔ {:+,.0f}'.format(
    ...
    def _print_daily_brief(self, signal):
        ...
        # 行1: 核心指标
        self._log('[SIGNAL] {} | SignalBase Y{:.2f} (source: {}) | ATR {:.1f}% | RSI {:.0f} | Vol_ratio {} | Mult {:.2f} | Trig Y{:.2f}{} {}'.format(
        ...
        if signal.get('quantile_trend_active'):
        ...
        if signal.get('quantile_trend_active'):
            sell_formula = ('plan sell Y{sell:.2f} = open Y{open:.2f}*'
                            '(1+ATR {atr:.2f}%*quantile_units {units:.3f})').format(
        ...
        if guard_active:
            ...
            self._log('[REV-T] ENABLED {} lots | {}'.format(short_lots, rev_plan))
        else:
        ...
        fwd_plan = ('plan buy Y{buy:.2f} = max(floor Y{floor:.2f}, trail '
                    ...
                    'Y{buy:.2f}*(1+{rise:.1f}%) | plan 1 lot~Y{lot:,.0f}').format(
                        ...
                        lot=buy_trig * self.trade_lot)
        if guard_active:
            self._log('[FWD-T] FROZEN near upper limit; capacity {} lots | {}'.format(
                long_lots, fwd_plan))
        elif do_long:
            self._log('[FWD-T] ENABLED {} lots | {}'.format(long_lots, fwd_plan))
        else:
            self._log('[FWD-T] BLOCKED {} | {}'.format(
                self.st.get('long_reason', 'unknown'), fwd_plan))

        if MOM_ENABLED:
            self._log('[MOM] {}minutes±{:.1f}×ATR in 10 minutes({:.1f}%~{:.1f}%) 反T买回-{:.1f}% 正T卖回+{:.1f}% self.trade_lot({}sh) 上限{}次 紧急买回{}'.format(
    ...
    def run(self):
        ...
        try:
            while self._running:
                ...
                tick = self.ctx.get_full_tick([self.stock_qmt])
                if self.stock_qmt not in tick: (yield 1); continue
                tick_data = tick[self.stock_qmt]
                price = tick_data.get('lastPrice', 0)
                if price <= 0: (yield 1); continue
                self._update_intraday_average(tick_data)
                self._update_limit_up_guard(price, tick_data, now_ts)
                ...
                (yield 0.5)
        except KeyboardInterrupt: self._log('Interrupted by user')
            ...
            self._log('[STOP] {} v45 cum {} days gross~Y{:,.0f}'.format(self.stock_name, self.total_t_days, self.total_pnl))
...
class PortfolioRunner:
    ...
    def refresh_holdings(self, now):
        if self.last_refresh is not None and now - self.last_refresh < PORTFOLIO_REFRESH_SEC:
            return
            ...
            while True:
                ...
                for code, (task, due) in list(self.tasks.items()):
                    ...
                    try:
                        ...
                        del self.tasks[code]
                        _log('[PORTFOLIO-ALERT] {} worker stopped; inspect its error log'.format(code))
                _time.sleep(0.2)
...
def main():
    parser = argparse.ArgumentParser(description='v45: account-wide trend-quantile T and connection monitor')
    parser.add_argument('--mode', default='signal', choices=['signal', 'live'])
    args = parser.parse_args()
    logger = FileLogger('portfolio', version='v45')
    set_logger(logger)
    try:
        if args.mode == 'live':
