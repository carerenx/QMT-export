...
"""
================================================================================
 MiniQMT — QMT day trading v46：账户全部持仓做T + 平仓成交价锚定ATR重入
================================================================================

 [v46 改动] (基于 v45 独立策略代码)
 ...
 [run mode]
 python "Stragety/MiniQMT_Stragety/DayT/DayT_v46_ATRReentry.py" --mode signal
 python "Stragety/MiniQMT_Stragety/DayT/DayT_v46_ATRReentry.py" --mode live

 python run_bigqmt.py --strategy Stragety/MiniQMT_Stragety/DayT/DayT_v46_ATRReentry.py --mode live
================================================================================
"""
...
# Permit the documented ``python Stragety/.../DayT_v46...py`` command.
...
class StrategyRunner:
    ...
    def _wait_for_fill(self, snap_before, expected_shares_delta,
        ...
        self.conn.cancel_order(self._submitted_order_id)
        return 'TIMEOUT', 0
    ...
    def run(self):
        ...
        try:
            ...
            self._log('[STOP] {} v46 cum {} days gross~Y{:,.0f}'.format(self.stock_name, self.total_t_days, self.total_pnl))
...
def main():
    parser = argparse.ArgumentParser(description='v46: account-wide trend-quantile T and connection monitor')
    parser.add_argument('--mode', default='signal', choices=['signal', 'live'])
    args = parser.parse_args()
    logger = FileLogger('portfolio', version='v46')
    set_logger(logger)
    try:
        if args.mode == 'live':
