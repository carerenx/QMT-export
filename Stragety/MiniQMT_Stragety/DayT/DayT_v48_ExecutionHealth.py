...
"""
================================================================================
 MiniQMT — QMT day trading v48：账户做T：统一ATR、订单成交核算与独立运行监测
================================================================================

 [v48 改动] (基于 v47 独立策略代码)
 ...
 [run mode]
 python "Stragety/MiniQMT_Stragety/DayT/DayT_v48_ExecutionHealth.py" --mode signal
 python "Stragety/MiniQMT_Stragety/DayT/DayT_v48_ExecutionHealth.py" --mode live

 python run_bigqmt.py --strategy Stragety/MiniQMT_Stragety/DayT/DayT_v48_ExecutionHealth.py --mode live
================================================================================
"""
...
# Permit the documented ``python Stragety/.../DayT_v48...py`` command.
...
class StrategyRunner:
    ...
    def _print_daily_brief(self, signal):
        ...
        if guard_active:
            ...
            self._log('[REV-T] ENABLED plan {} sh / remaining {} entries | {}'.format(planned_short, remaining_short, rev_plan))
        else:
        ...
        if guard_active:
            ...
            self._log('[FWD-T] ENABLED plan {} sh / remaining {} entries | {}'.format(planned_long, remaining_long, fwd_plan))
        else:
    ...
    def _refresh_execution_capacity(self, force=False):
        now = _time.monotonic()
        if not force and now - self._last_capacity_refresh < CAPACITY_REFRESH_SEC:
            return
    ...
    def run(self):
        ...
        try:
            ...
            self._log('[STOP] {} v48 cum {} days gross~Y{:,.0f}'.format(self.stock_name, self.total_t_days, self.total_pnl))
...
def main():
    parser = argparse.ArgumentParser(description='v48: portfolio T with verified executions, unified ATR and runtime watchdog')
    parser.add_argument('--mode', default='signal', choices=['signal', 'live'])
    args = parser.parse_args()
    logger = FileLogger('portfolio', version='v48')
    set_logger(logger)
    try:
        if args.mode == 'live':
