# -*- coding: utf-8 -*-
"""
v52 DirectionalOvernight — independent v51 execution snapshot, research only.
...
STATE_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'state', 'v52_{}.json'.format(cfg.ACCOUNT))
...
def main():
    parser = argparse.ArgumentParser(description='v52: unified minute-strength reference; shadow by default, verified ledger recovery')
    parser.add_argument('--mode', default='signal', choices=['signal', 'live'])
    args = parser.parse_args()
    if args.mode == 'live':
        raise RuntimeError('v52 RESEARCH ONLY: strict recovery and out-of-sample acceptance pending; live disabled')
    if INTRADAY_REFERENCE_MODE not in ('shadow', 'active'):
        raise ValueError('INTRADAY_REFERENCE_MODE must be shadow or active')
    logger = FileLogger('portfolio', version='v52')
    set_logger(logger)
    try:
        if args.mode == 'live':
