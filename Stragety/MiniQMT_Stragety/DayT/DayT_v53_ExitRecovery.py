# -*- coding: utf-8 -*-
"""
v53 ExitRecovery — independent no-MOM strategy with bounded exit intent.
Live requires explicit confirmation; no guarantee of profitability or repurchase.
"""
# v52研究参数：只限制新增正向周期，旧周期退出不受方向准入影响。
DIRECTIONAL_THRESHOLD = 0.20  # 候选仅 0 / 0.20 / 0.40；不得按留出结果继续调参。
DIRECTIONAL_ENABLED = True
LONG_RESEARCH_DISABLED = False  # 仅用于真实重跑共享资金的关闭正向对照，不是上线默认。
OVERNIGHT_ENABLED = True
MAX_OPEN_CYCLES = 2  # 主策略和MOM合计；阶梯属于原周期。
...
# 买回目标经过30个连续有效分钟间隔后收紧并锁存；午休/断流不累计。
EXIT_TIGHTEN_MINUTES = 30
EXIT_TIGHTEN_MULT = 0.60  # 原ATR买回距离保留60%，不参考原始持仓成本。
EXIT_RETRY_MAX = 3  # 首次委托计入；达到上限保留退出意图并告警，需人工处理。
EXIT_CHASE_ATR_UNITS = 0.05  # 更新限价不高于首次退出触发价*(1+入场ATR*此值)。
EXIT_TRAIL_ATR_UNITS = 0.0  # 0关闭；候选仅0.03/0.05/0.08，先回测再启用。
CONFIRM_FLAT_START = False  # 仅由首次实盘CLI明确确认；不绕过已有检查点核对。
...
from core.exit_recovery import ExitRecovery
...
STATE_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'state', 'v53_{}.json'.format(cfg.ACCOUNT))
...
# MOM/REV-T buybacks use an explicit ask1 limit to avoid peer-price
...
MOM_ATR_MULT          = 3.6           # 触发幅度 = 2 × 最近10分钟ATR (自适应)
...
# REV-T临近触发区时MOM让权（独立开关）
MOM_REV_PRIORITY_ENABLED  = True       # True=MOM接近REV-T触发区时停止卖出并让权
MOM_REV_PRIORITY_BAND_PCT = 0.012      # REV-T触发价下方1.2%起进入让权区

# MOM独立、波动自适应回撤确认,回落冻结（独立开关）
MOM_ADAPTIVE_PULLBACK_ENABLED = True   # False=恢复使用cfg.PULLBACK_PCT
MOM_PULLBACK_ATR_MULT         = 0.30   # 回撤阈值取最近分钟波幅ATR的30%
MOM_PULLBACK_MIN_PCT          = 0.0020 # 下限0.20%，过滤高价股盘口微回撤
MOM_PULLBACK_MAX_PCT          = 0.0060 # 上限0.60%，限制峰值让利

# MOM平仓确认冷却（首次12秒，续冻每轮递减2秒，最低2秒）
...
class ExecutionRunner:
    ...
    def restore_record(self, record):
        ...
        self._restored = True
        self._log('[STATE-RESTORED] ledger state={} REV-count={} FWD-count={} MOM-count={}'.format(
    ...
    def _init_state(self):
        self.st.update({
            ...
            # ★ v26: 自适应ATR — 10分钟价格窗口 + 当前触发幅度
            'mom_atr_history': deque(), 'mom_trigger_pct': 0.0,
            # ★ v33: MOM让权状态 + 建仓时冻结的独立回撤确认阈值
    ...
    def _recalculate_next_t_triggers(self, completed_by):
        ...
        # 暂时不可用，也不能让旧监测继续形成下一笔T；已有成交腿不动。
        if completed_by.startswith('MOM'):
            if st.get('fstate') == STATE_SPIKING:
    ...
    def _handle_sold(self, price):
        ...
        tightened_bt = max(bt, self.exit_recovery.state.get('target', bt))
        ...
        # ★ v35: 99%二次回升买回。
        # 仅在原买回价低于99%线时才可能成立；首次向下到线只锁存，
        # 随后未触及原买回价便向上回到该线，才执行卖一价FIX买回。
    ...
    def _mom_should_yield_to_rev(self, price):
        """MOM卖出点进入REV-T优先区时让权，避免同一上涨波段重复卖出。"""
    ...
    def _mom_handle_rev_yield(self, price):
        """保持让权；达到REV阈值立即释放，向下退出优先区则在主REV-T空闲后释放。"""
    ...
    def _mom_status(self):
        ...
        if ms == MOM_STATE_BUYBACK_COOLING:
            return '买回冷却: trigger Y{:.2f} cycle {} / {:.0f}s'.format(
        ...
        if ms == MOM_STATE_SELLBACK_COOLING:
            return '卖回冷却: trigger Y{:.2f} cycle {} / {:.0f}s'.format(
                st.get('mom_cooldown_trigger', 0.0), st.get('mom_cooldown_cycles', 0),
                st.get('mom_cooldown_duration', 0.0))
        if ms == MOM_STATE_REV_YIELD:
            return '已让权REV-T trig Y{:.2f}'.format(st.get('mom_rev_yield_trigger', 0.0))
        return ms
    ...
    def _mom_handle_spiking(self, price):
        """MOM反T冲高回落卖出：独立回撤确认，并在REV-T优先区让权。"""
        st = self.st
        ...
        if MOM_EMERGENCY_BUYBACK_ENABLED and price >= sp * (1.0 + cfg.EMERGENCY_BUYBACK_PCT):
            ...
            if bought > 0:
                ...
                if bought >= shares:
                    ...
                    self._recalculate_next_t_triggers('MOM REV-T')
                else:
    ...
    def _mom_handle_buyback_cooling(self, price, now_ts=None):
        """冷却到期后，仅在买回触发价与卖价-0.5%之间买回，否则续冻。"""
        st = self.st
        now = _time.time() if now_ts is None else now_ts
    ...
    def _mom_handle_sellback_cooling(self, price, now_ts=None):
        """冷却到期后，仅在买价+0.5%与卖回触发价之间卖出，否则续冻。"""
        st = self.st
        now = _time.time() if now_ts is None else now_ts
        if now < st.get('mom_cooldown_until', 0.0):
            return
        ...
        if price < trigger and price > profit_floor:
            ...
            st['mom_pullback_pct'] = 0.0
            self._mom_clear_close_cooldown()
            self._recalculate_next_t_triggers('MOM FWD-T')
            return
    ...
    def _mom_force_close(self, price):
        ...
        if ms in ('MOM_SOLD', MOM_STATE_BUYBACK_COOLING):
            ...
            if bought >= shares:
                st['mom_state'] = 'MOM_IDLE'
                st['mom_sell_price'] = 0.0; st['mom_leg_shares'] = 0
                self._recalculate_next_t_triggers('MOM REV-T force')
            elif bought > 0:
                st['mom_leg_shares'] = shares - bought
                st['mom_state'] = 'MOM_SOLD'
            ...
            if sold >= shares:
                st['mom_state'] = 'MOM_IDLE'
                st['mom_buy_price'] = 0.0; st['mom_leg_shares'] = 0
                self._recalculate_next_t_triggers('MOM FWD-T force')
            elif sold > 0:
                st['mom_leg_shares'] = shares - sold
                st['mom_state'] = 'MOM_BT_BOUGHT'
    ...
    def run(self):
        ...
        try:
            ...
            self._log('[STOP] {} v52 cum {} days gross~Y{:,.0f}'.format(self.stock_name, self.total_t_days, self.total_pnl))
...
class StrategyRunner(ExecutionRunner):
    """Research lane: one independently owned main/MOM cycle per lane."""
    def __init__(self, portfolio, stock_qmt, stock_name='', lane=0):
        ...
        self.exit_recovery = ExitRecovery()
        self._exit_cycle_id = None
        self._exit_quote = None
        self._exit_alert = None

    def checkpoint_record(self):
        if self.cycle:
            self.cycle.targets = {key: self.st.get(key) for key in (
                'buyback_target', 'bt_sellback_target', 'mom_buyback_target',
                'mom_sellback_target', 'ladder_sell_target', 'ladder_buy_target')}
        ...
        record['exit_recovery'] = dict(cycle_id=self._exit_cycle_id, state=self.exit_recovery.state)
        return record
    ...
    def restore_record(self, record):
        ...
        state_quantity = sum(q for _, q in self.st.get('short_legs', [])) + sum(q for _, q in self.st.get('long_legs', [])) + self.st.get('mom_leg_shares', 0)
        ...
        extra_exit = record.get('exit_recovery')
        if self.cycle and self.cycle.quantity and extra_exit is None:
            raise RuntimeError('v53 active cycle requires exit recovery checkpoint; no implicit upgrade')
        self.exit_recovery = ExitRecovery((extra_exit or {}).get('state'))
        self._exit_cycle_id = (extra_exit or {}).get('cycle_id')
        if self.cycle and self.cycle.direction == 'SHORT' and self._exit_cycle_id != self.cycle.cycle_id:
            raise RuntimeError('exit intent/cycle ownership mismatch')
        self._exit_quote = None
    ...
    def _handle_sold(self, price):
        self._observe_exit(price)
        s = self.exit_recovery.state
        if s.get('pending'):
            self._do_buyback(price, 'EXIT_PENDING')
            return
        if s.get('trail_armed') and price >= s['low'] * (1 + cfg.BOUNCE_PCT):
            self._do_buyback(price, 'ATR_TRAIL')
            return
        return self._ladder_call(super()._handle_sold, price, 'REV-T sell')

    def _handle_spiking(self, price):
        result = self._ladder_call(super()._handle_spiking, price, 'REV-T sell')
        self._ensure_exit_owner()
        return result

    def _ensure_exit_owner(self):
        if self.cycle and self.cycle.direction == 'SHORT' and self._exit_cycle_id != self.cycle.cycle_id:
            self._exit_cycle_id = self.cycle.cycle_id
            self.exit_recovery = ExitRecovery()
            self._exit_quote = None
            self._exit_alert = None

    def _observe_exit(self, price):
        self._ensure_exit_owner()
        if not self.cycle or self.cycle.direction != 'SHORT':
            return
        minute = int(_time.time() // 60)
        pending = self._exit_quote
        if pending and minute != pending[0] and minute - pending[0] == 1:
            old_target = self.exit_recovery.state.get('target')
            self.exit_recovery.observe(pending[0], pending[1], self.st['sell_fill_price'],
                self.cycle.atr, self.st['buyback_target'], EXIT_TIGHTEN_MINUTES,
                EXIT_TIGHTEN_MULT, EXIT_TRAIL_ATR_UNITS)
            if old_target != self.exit_recovery.state.get('target'):
                self._log('[EXIT-TARGET] original={} effective={} elapsed={}'.format(
                    self.st['buyback_target'], self.exit_recovery.state.get('target'),
                    self.exit_recovery.state.get('elapsed', 0)))
        self._exit_quote = (minute, price)
    ...
    def _do_buyback(self, price, reason=''):
        ...
        if not self.cycle or self.cycle.direction != 'SHORT':
            return 0
        self.exit_recovery.begin(price, self.cycle.atr, EXIT_CHASE_ATR_UNITS)
        # Preserve the original fill accounting and partial-fill cleanup.
        return super()._do_buyback(price, reason)

    def _submit_buyback_order(self, shares, fallback_price, label):
        if not label.startswith('REV-T') or not self.exit_recovery.state.get('pending'):
            return super()._submit_buyback_order(shares, fallback_price, label)
        limit_price = self._buyback_limit_price(fallback_price)
        if not self.exit_recovery.permit(int(_time.time() // 60), limit_price, EXIT_RETRY_MAX):
            reason = self.exit_recovery.state.get('alert')
            if reason and reason != self._exit_alert:
                self._exit_alert = reason
                self._log('[EXIT-ALERT] {} cap={} attempts={}; intent retained, manual review required'.format(
                    reason, self.exit_recovery.state['cap'], self.exit_recovery.state['attempts']))
            return 'SKIP', 0
        if self.portfolio.order_uncertain:
            return 'SKIP', 0
        self._last_buyback_price = limit_price
        self._log('[EXIT-PENDING] attempt={} cap={} limit={} remaining={}'.format(
            self.exit_recovery.state['attempts'], self.exit_recovery.state['cap'], limit_price, shares))
        status, delta = self._submit_order(shares, limit_price, label, style='FIX')
        if delta:
            self._last_buyback_price = self._execution_price
        return status, delta
    ...
    def _submit_order(self, shares, price, label, style='COMPETE'):
        ...
        if self.paused_reason:
            ...
            return 'SKIP', 0
        if opening:
            if LONG_RESEARCH_DISABLED and label in ('FWD-T buy', 'MOM long'):
                return 'SKIP', 0
            if self._new_leg_block_reason():
                return 'SKIP', 0
            today = datetime.now().strftime('%Y%m%d')
            ...
            count = sum(record['opened_day'] == today and (
                record.get('label', '').startswith('MOM') if label.startswith('MOM')
                else record.get('label') == label) for record in records)
            count += sum(sibling.imported_entry_counts.get(today, {}).get(
                'MOM' if label.startswith('MOM') else label, 0)
                for sibling in self.portfolio.symbol_runners(self.stock_qmt))
            limit = MOM_MAX_DAILY_TRADES if label.startswith('MOM') else cfg.MAX_DAILY_TRADES
            if count >= limit:
                self._log('[CYCLE-BLOCKED] shared symbol daily entry limit reached')
                return 'SKIP', 0
            if not ladder and label in ('FWD-T buy', 'MOM long') and DIRECTIONAL_ENABLED and not self.admission.allowed:
    ...
    def restore_checkpoint(self):
        ...
        if not saved:
            ...
            if saved:
                ...
                for code, record in saved['runners'].items():
                    state = record['state']
                    if (any(record['legs'].values()) or state.get('short_legs') or state.get('long_legs')
                            or state.get('mom_leg_shares', 0)):
                    ...
                    record['v52'] = dict(lane=0, baseline=positions.get(code, 0),
                        cycle=None, history=[], paused='', imported_counts={state['trade_date']: {
                            ...
                            'MOM': state.get('mom_trade_count', 0)}})
                _log('[UPGRADE] flat legacy ledger imported read-only; old file unchanged')
            else:
                if not CONFIRM_FLAT_START:
                    raise RuntimeError('v53 first start requires --confirm-flat-start after reconciling all old T legs')
                self.checkpoint_active = True
                return
        if saved.get('inflight') or saved.get('order_uncertain'):
...
def main():
    global CONFIRM_FLAT_START
    parser = argparse.ArgumentParser(description='v53 no-MOM bounded exit recovery; signal by default')
    parser.add_argument('--mode', default='signal', choices=['signal', 'live'])
    parser.add_argument('--confirm-flat-start', action='store_true', help='confirm no unrecorded T legs; does not bypass saved account audit')
    args = parser.parse_args()
    CONFIRM_FLAT_START = args.confirm_flat_start
    if INTRADAY_REFERENCE_MODE not in ('shadow', 'active'):
        raise ValueError('INTRADAY_REFERENCE_MODE must be shadow or active')
    logger = FileLogger('portfolio', version='v53')
    set_logger(logger)
    try:
        if args.mode == 'live':
            print('WARNING: research parameters; pending exits may require manual handling; no profit guarantee. Stop other strategies on this account first.')
