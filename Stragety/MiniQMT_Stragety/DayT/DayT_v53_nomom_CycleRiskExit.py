# -*- coding: utf-8 -*-
"""
v53 CycleRiskExit — v52_nomom with bounded unfinished REV-T risk, research only.
...
CYCLE_ADVERSE_ATR_MULT = 0.75
CYCLE_MIN_ADVERSE_PCT = 0.03
...
class ExecutionRunner:
    """MiniQMT v53 — bounded-cycle-risk research runner."""
    ...
    def run(self):
        ...
        try:
            ...
            self._log('[STOP] {} v53 cum {} days gross~Y{:,.0f}'.format(self.stock_name, self.total_t_days, self.total_pnl))
...
def main():
    parser = argparse.ArgumentParser(description='v53: v52_nomom with bounded cycle risk exits; research only')
    parser.add_argument('--mode', default='signal', choices=['signal', 'live'])
    args = parser.parse_args()
    if args.mode == 'live':
        raise RuntimeError('v53 RESEARCH ONLY: out-of-sample acceptance pending; live disabled')
