# -*- coding: utf-8 -*-
"""
v54 TrendGuard — v53 with a three-session trend gate for new REV-T cycles.
...
STATE_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'state', 'v54_{}.json'.format(cfg.ACCOUNT))
...
class ExecutionRunner:
    """MiniQMT v54 — v53 risk exits plus a completed-day trend guard."""
    ...
    def run(self):
        ...
        try:
            ...
            self._log('[STOP] {} v54 cum {} days gross~Y{:,.0f}'.format(
...
def main():
    parser = argparse.ArgumentParser(description='v54: v53 with completed-day REV-T trend guard; research only')
    parser.add_argument('--mode', default='signal', choices=['signal', 'live'])
    args = parser.parse_args()
    if args.mode == 'live':
        raise RuntimeError('v54 RESEARCH ONLY: out-of-sample acceptance pending; live disabled')
    if INTRADAY_REFERENCE_MODE not in ('shadow', 'active'):
        raise ValueError('INTRADAY_REFERENCE_MODE must be shadow or active')
    logger = FileLogger('portfolio', version='v54')
    set_logger(logger)
    try:
        if args.mode == 'live':
