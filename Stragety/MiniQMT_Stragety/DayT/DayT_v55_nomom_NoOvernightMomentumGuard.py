# -*- coding: utf-8 -*-
"""
v55 NoOvernightMomentumGuard — v53 with a three-session trend gate for new REV-T cycles.
...
STATE_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'state', 'v55_{}.json'.format(cfg.ACCOUNT))
...
class ExecutionRunner:
    """MiniQMT v55 — v54 plus no-overnight and five-day momentum guards."""
    ...
    def run(self):
        ...
        try:
            ...
            self._log('[STOP] {} v55 cum {} days gross~Y{:,.0f}'.format(
...
def main():
    parser = argparse.ArgumentParser(description='v55: v54 with no-overnight and momentum guards; research only')
    parser.add_argument('--mode', default='signal', choices=['signal', 'live'])
    args = parser.parse_args()
    if args.mode == 'live':
        raise RuntimeError('v55 RESEARCH ONLY: out-of-sample acceptance pending; live disabled')
    if INTRADAY_REFERENCE_MODE not in ('shadow', 'active'):
        raise ValueError('INTRADAY_REFERENCE_MODE must be shadow or active')
    logger = FileLogger('portfolio', version='v55')
    set_logger(logger)
    try:
        if args.mode == 'live':
