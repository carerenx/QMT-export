# -*- coding: utf-8 -*-
"""
v57 RestartGuard: v56 signals with fail-closed intraday/cross-day restart admission.
Direction admission and serialized cycle ownership are under validation.
Live mode is disabled. This strategy never auto-imports legacy checkpoints.
Missing state or broker-side changes require explicit manual reconciliation.
"""
# v52研究参数：只限制新增正向周期，旧周期退出不受方向准入影响。
DIRECTIONAL_THRESHOLD = 0.20  # 候选仅 0 / 0.20 / 0.40；不得按留出结果继续调参。
DIRECTIONAL_ENABLED = True
LONG_RESEARCH_DISABLED = True  # v57：正T消融为负，冻结该通道直至独立样本验证通过。
...
CYCLE_ADVERSE_ATR_MULT = 0.10
CYCLE_MIN_ADVERSE_PCT = 0.01
SHORT_SESSION_EXIT_TIME = '14:50:00'
...
SHORT_ADVERSE_COOLDOWN_SEC = 3600.0
...
from core.restart_guard import validate_restart, RestartBlocked
...
def short_session_exit_reason(now_hms, cutoff=SHORT_SESSION_EXIT_TIME):
    """Return the deterministic close-out reason at or after the cutoff."""
    return 'SESSION_END' if now_hms >= cutoff else ''
...
class ExecutionRunner:
    """MiniQMT v57 — confirmed reversals with a protected core position."""
    ...
    def _handle_idle(self, price):
        ...
        if st.get('do_short', False):
            if cfg.now_hms() >= SHORT_NEW_ENTRY_CUTOFF:
                return
            if datetime.now().timestamp() < st.get('short_cooldown_until', 0.0):
                return
    ...
    def run(self):
        ...
        try:
            ...
            self._log('[STOP] {} v57 cum {} days gross~Y{:,.0f}'.format(
...
class ExecutionPortfolio:
    ...
    def save_checkpoint(self, force=False, settled=False):
        ...
        data = dict(schema=1, account=str(ACCOUNT), strategy="v57_restart_guard",
    ...
    def before_submit(self, code, label, shares, price):
        if not self.checkpoint_active:
            if not self.dry_run:
                raise RestartBlocked("RESTART-BLOCKED: account not admitted")
            return
        if self.order_uncertain or self.inflight:
            raise RestartBlocked("RESTART-BLOCKED: previous intent unresolved")
...
class StrategyRunner(ExecutionRunner):
    ...
    def _force_buyback(self):
        if not self.cycle or not self.st.get('short_legs'):
            self._log('[FORCE buyback SKIP] cycle already flat')
            return None
        return super()._force_buyback()
    ...
    def _short_cycle_risk_reason(self, price):
        ...
        if average <= 0 or price <= 0:
            return ''
        session_reason = short_session_exit_reason(
            datetime.now().strftime('%H:%M:%S'))
        if session_reason:
            return session_reason
        adverse_pct = price / average - 1.0
        adverse_limit = max(
            CYCLE_MIN_ADVERSE_PCT,
            float(cycle.atr or 0.0) * CYCLE_ADVERSE_ATR_MULT)
        if adverse_pct >= adverse_limit:
            return 'ADVERSE_MOVE'
    ...
    def _risk_buyback(self, price, reason):
        ...
        if bought >= shares:
            ...
            if reason == 'ADVERSE_MOVE':
                st['short_cooldown_until'] = (
                    datetime.now().timestamp() + SHORT_ADVERSE_COOLDOWN_SEC)
...
class PortfolioRunner(ExecutionPortfolio):
    ...
    def _broker_snapshot(self):
        snapshot = super()._broker_snapshot()
        account = self.conn.query_account()
        trades = self.conn.trader.query_stock_trades(self.conn._account_obj)
        if account is None or trades is None:
            raise RestartBlocked('RESTART-BLOCKED: account/trade query unavailable')
        snapshot['cash'] = float(account.cash)
        rows = {}
        for trade in trades:
            trade_id = str(getattr(trade, 'traded_id', '') or '')
            order_id = str(getattr(trade, 'order_id', '') or '')
            code = str(getattr(trade, 'stock_code', '') or '')
            if not trade_id or not order_id or not code:
                raise RestartBlocked('RESTART-BLOCKED: execution has no stable identity')
            key = code + '|' + order_id + '|' + trade_id
            row = [key, str(getattr(trade, 'traded_time', '')),
                   int(trade.traded_volume), float(trade.traded_price)]
            if key in rows and rows[key] != row:
                raise RestartBlocked('RESTART-BLOCKED: conflicting duplicate execution')
            rows[key] = row
        snapshot['trades'] = sorted(rows.values())
        return snapshot

    def restore_checkpoint(self):
        if self.dry_run:
            return
        # Never import v49/v56 state or infer a fresh baseline from current shares.
        saved = read_checkpoint(STATE_FILE, ACCOUNT)
        first = self._broker_snapshot()
        current = self._broker_snapshot()
        if first != current:
            raise RestartBlocked('RESTART-BLOCKED: broker changed during reconciliation')
        mode = validate_restart(saved, current, datetime.now().strftime('%Y%m%d'))
        restored = {}
        tasks = {}
        for key, record in saved['runners'].items():
            ...
            runner.restore_record(record)
            restored[key] = runner
            tasks[key] = (runner.run(), 0.0)
        # Commit the reconstructed graph only after every record validates.
        self.runners = restored
        self.tasks = tasks
        self.own_order_ids = set(saved.get('own_order_ids', []))
        self.checkpoint_active = True
        _log('[RESTART-ADMITTED] {}: account, executions and cycle ownership checked'.format(mode))


def main():
    parser = argparse.ArgumentParser(description='v57: confirmed reversals and protected core position; research only')
    parser.add_argument('--mode', default='signal', choices=['signal', 'live'])
    args = parser.parse_args()
    if args.mode == 'live':
        raise RuntimeError('v57 RESEARCH ONLY: out-of-sample acceptance pending; live disabled')
    if INTRADAY_REFERENCE_MODE not in ('shadow', 'active'):
        raise ValueError('INTRADAY_REFERENCE_MODE must be shadow or active')
    logger = FileLogger('portfolio', version='v57')
    set_logger(logger)
    try:
        if args.mode == 'live':
