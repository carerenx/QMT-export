...
"""
================================================================================
 MiniQMT — QMT day trading v14 + 日志系统
================================================================================
 VS Code connect xtquant MiniQMT run v14 strategy。

 [v14 新增]
   ★ 日志系统: 策略运行时自动创建带时间戳的日志文件
   ★ 所有 _log() 输出同时写入控制台 + 文件
   ★ 日志文件路径: ./logs/DayTradeing_v14_601869_20260807_093000.log
   ★ 每次 _log() 调用后立即 flush, 确保实时持久化
   ★ 优雅退出时打印日志文件路径
 ...
 [arch]
   VS Code (script)
     ...
     ├─ v14strategy → signal + state machine (保持与QMT内置版完全一致)
     └─ FileLogger  → 日志文件实时写入
 ...
 [run mode]
   ...
   mode1 — signal mode(only monitor):
     python "Stragety/MiniQMT_Stragety/DayTradeing_v14_stragety_miniqmt.py" --mode signal

   mode2 — real mode (auto order):
     python "Stragety/MiniQMT_Stragety/DayTradeing_v14_stragety_miniqmt.py" --mode live

   mode3 — backtest mode (用QMT本地数据回测, 不需要MiniQMT):
     python "Stragety/MiniQMT_Stragety/DayTradeing_v14_stragety_miniqmt.py" --mode backtest

 [diff with v13]
   v13: 仅控制台 print 输出
   v14: ★ 新增 FileLogger, 所有日志写入文件 + 控制台, 文件名含时间戳
...
class FileLogger:
    ...
    特性:
      - 文件名包含策略版本+标的+启动时间, 如 DayTradeing_v14_601869_20260807_093000.log
      - 每次 _log() 调用后立即 flush, 崩溃也不丢失日志
      - 日志目录自动创建 (./logs/)
      - 支持 UTF-8 编码 (兼容中文)
    """

    def __init__(self, stock_code='601869', log_dir=None):
        """
        Args:
            stock_code: 标的代码, 用于日志文件名
            log_dir:    日志目录, 默认为策略文件同级的 logs/ 目录
        """
        ...
        self.log_filename = f'DayTradeing_v14_{stock_code}_{start_time}.log'
        ...
        self._file.write(f'  策略版本: v14 (MiniQMT)\n')
        self._file.write(f'  标的: {stock_code}\n')
        self._file.write(f'{"="*60}\n')
        self._file.flush()
...
# ── ★ v14: _log 改为双写 (控制台 + 文件) ──
#     通过全局 _global_logger 引用 FileLogger 实例

_global_logger: Optional[FileLogger] = None


def _log(*args, **kwargs):
    """输出日志到控制台 + 文件 (如果已初始化 FileLogger)"""
    ts = _ts()
    ...
    # 传递原始关键字参数给底层写入
    if kwargs and _global_logger:
        _global_logger.write(**kwargs)


def _write_log(msg):
    """写入控制台 + 文件"""
    if _global_logger is not None:
        _global_logger.write(msg + '\n')
    else:
        print(msg)
...
class StrategyRunner:
    ...
    def __init__(self, dry_run=False):
        global _global_conn, _global_dry_run, _global_logger

        # ★ v14: 初始化日志系统 (必须在其他操作之前)
        if _global_logger is None:
            _global_logger = FileLogger(STOCK_CODE)
    ...
    def _reset_daily(self):
        ...
        self.st['lock_since'] = ''
        self.st['bt_sell_peak_price'] = 0.0
        self.st['_pre_market_done'] = ''   # v14: 重置盘前初始化标记
    ...
    def _print_signal(self, signal):
        ...
        _log('')
        _log('╔' + '═' * 55 + '╗')
        _log('║  {}  迷你反T v14  |  {}  |  {}  ║'.format(STOCK_NAME, trade_date, trend))
        _log('╠' + '═' * 55 + '╣')
    ...
    def run(self):
        ...
        # 初始化状态
        self._init_state()
        _log(f'{STOCK_NAME} 迷你反T v14 MiniQMT版 启动')
        ...
        _log(f'  ★ 日志文件: {_global_logger.log_path_str()}')
        ...
        try:
            while self._running:
                ...
                if not _is_market_open(now):
                    ...
                    # ★ v14: 9:25集合竞价结束 → 提前计算当日信号
                    #       此时开盘价已确定, 可以提前算好触发线/仓位,
                    #       9:30连续竞价一开始策略就能直接进入监控状态
                    if '09:25:00' <= now < '09:30:00':
                        if self.st.get('_pre_market_done', '') != today:
                            _log(f'[盘前预初始化] {now} 集合竞价结束, 开盘价已定, 提前计算信号...')
                            try:
                                self._daily_init()
                        ...
                        if now_ts - self._last_heartbeat >= 60:
                            self._last_heartbeat = now_ts
                            _log(f'[盘前就绪] {now} — 距开盘 {_time_to_open(now)}')
                        _time.sleep(5)
                        continue
            ...
            _log(f'{STOCK_NAME} v14 MiniQMT版 已停止 | 累计 {self.total_t_days}天 | 毛利~Y{self.total_pnl:,.0f}')

            # ★ v14: 关闭日志文件, 输出路径
            if _global_logger is not None:
                log_path = _global_logger.log_path_str()
                _write_log(f'★ 日志已保存至: {log_path}')
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='MiniQMT 迷你反T v14 策略 — VS Code 运行版 (含日志系统)',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
运行示例:
  python "Stragety/MiniQMT_Stragety/DayTradeing_v14_stragety_miniqmt.py" --mode signal    (信号监控)
  python "Stragety/MiniQMT_Stragety/DayTradeing_v14_stragety_miniqmt.py" --mode live      (实盘自动交易)
  python "Stragety/MiniQMT_Stragety/DayTradeing_v14_stragety_miniqmt.py" --mode backtest  (回测)

v14 新增:
  ★ 所有运行日志实时写入文件 (logs/ 目录下, 文件名含时间戳)
  ★ 控制台输出保持与 v13 一致
...
前置条件:
  ...
  3. QMT 已下载 601869 历史数据
    ...
    # ★ v14: 提前初始化日志系统 (在策略启动之前)
    _global_logger = FileLogger(STOCK_CODE)
    print(f'★ 日志文件: {_global_logger.log_path_str()}')
