...
"""
================================================================================
 MiniQMT — QMT day trading v17 + 交易后仓位资金校验
================================================================================
 VS Code connect xtquant MiniQMT run v17 strategy。

 [v17 改动] (vs v16)
   ★ 每次下单后实时查询持仓 & 资金, 对比交易前后变化并打印到日志
   ★ _snapshot_account()    — 交易前快照: 持仓/可用资金/总资产
   ★ _verify_trade()        — 交易后校验: 对比快照, 打印 Δ持仓/Δ资金
   ★ 覆盖全部6个下单点: 反T卖出/买回, 正T买入/卖出, 强平买回/卖出
 ...
 [run mode]
   mode1 — signal mode:
     python "Stragety/MiniQMT_Stragety/DayTradeing_v17_stragety_miniqmt.py" --mode signal
   mode2 — real mode:
     python "Stragety/MiniQMT_Stragety/DayTradeing_v17_stragety_miniqmt.py" --mode live
   mode3 — backtest mode:
     python "Stragety/MiniQMT_Stragety/DayTradeing_v17_stragety_miniqmt.py" --mode backtest
...
class StrategyRunner:
    """MiniQMT 策略运行器 — 状态机 + 主循环 + 锁仓 + 正反T + 交易后校验"""

    def __init__(self, dry_run=False):
        logger = get_logger()
        if logger is None:
            logger = FileLogger(STOCK_CODE, version='v17')
            set_logger(logger)
    ...
    def _snapshot_account(self):
        ...
        Returns:
            dict: {
                'shares': int,       总持仓股数
                'can_use': int,      T+0 可用股数
                'cash': float,       可用资金
                'cost': float,       持仓成本
                'total_asset': float, 总资产 (持仓市值 + 可用资金)
                'price': float,      当前市价 (用于估算市值)
            }
        """
        # 持仓
        positions = get_trade_detail_data(ACCOUNT, 'STOCK', 'POSITION')
        ...
        if price <= 0:
            ...
            price = signal.get('open_price', 0)

        total_asset = shares * price + cash

        return {
            'shares': shares, 'can_use': can_use, 'cash': cash,
            'cost': cost, 'total_asset': total_asset, 'price': price,
        }

    def _verify_trade(self, snap_before, label, trade_price, trade_shares):
        """
        ★ v17 新增: 交易后校验 — 对比交易前后快照, 打印仓位/资金变化。

        在下单后稍作等待 (0.5s) 再查询, 给 QMT 后台成交时间。

        Args:
            snap_before: _snapshot_account() 返回的交易前快照
            label:       交易描述 (如 '反T卖出', '反T买回(正常)')
            trade_price: 下单价格
            trade_shares: 下单股数 (正=买入, 负=卖出)
        """
        _time.sleep(0.5)  # 等待成交回报
        ...
        # 理论现金变化 (卖出+现金, 买入-现金, 含佣金印花税估算)
        est_cost = abs(trade_price * abs(trade_shares)) * (cfg.COMMISSION + cfg.STAMP_TAX)
    ...
    def _handle_spiking(self, price):
        ...
        if pullback >= cfg.PULLBACK_PCT:
            ...
            self._verify_trade(snap, '反T卖出', price, -TRADE_LOT_SIZE)
    ...
    def _do_buyback(self, price, reason=''):
        ...
        self._verify_trade(snap, f'反T买回({reason})', price, TRADE_LOT_SIZE)
    ...
    def _force_buyback(self):
        ...
        self._verify_trade(snap, '反T强制买回', 0, TRADE_LOT_SIZE)
    ...
    def _handle_bt_dipping(self, price):
        ...
        if bounce >= cfg.BOUNCE_PCT:
            ...
            self._verify_trade(snap, '正T买入', price, TRADE_LOT_SIZE)
    ...
    def _handle_bt_spiking(self, price):
        ...
        if pullback >= cfg.PULLBACK_PCT:
            ...
            self.total_t_days += 1
            self.total_pnl += gross
            self._verify_trade(snap, '正T卖出', price, -TRADE_LOT_SIZE)
    ...
    def _do_bt_force_sell(self):
        ...
        order_shares(STOCK_QMT, -TRADE_LOT_SIZE, 'COMPETE', 0, self.ctx, ACCOUNT)
        self.st['fstate'] = STATE_FORCED
        self._verify_trade(snap, '正T强制卖出', 0, -TRADE_LOT_SIZE)
    ...
    def run(self):
        ...
        self._init_state()
        _log(f'{STOCK_NAME} 迷你反T v17 MiniQMT版 启动')
        ...
        _log(f'  ★ 模块: core/ + infra/ 拆分结构 (v17 交易后校验)')
        ...
        try:
            ...
            _log(f'{STOCK_NAME} v17 MiniQMT版 已停止 | 累计 {self.total_t_days}天 | 毛利~Y{self.total_pnl:,.0f}')
...
def run_backtest_mode(start='20250801', end='20260806'):
    ...
    print(f'\n{"="*55}')
    print(f'  回测mode — QMT 迷你反T v17 (模块化)')
...
def main():
    parser = argparse.ArgumentParser(
        description='MiniQMT 迷你反T v17 策略 — 交易后仓位资金校验版',
