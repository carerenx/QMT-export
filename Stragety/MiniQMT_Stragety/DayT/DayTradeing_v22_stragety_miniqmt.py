...
"""
================================================================================
 MiniQMT — QMT day trading v22 + 阶梯加仓机制
================================================================================
 ...
 [run mode]
 python "Stragety/MiniQMT_Stragety/DayTradeing_v22_stragety_miniqmt.py" --mode signal
 python "Stragety/MiniQMT_Stragety/DayTradeing_v22_stragety_miniqmt.py" --mode live
 python "Stragety/MiniQMT_Stragety/DayTradeing_v22_stragety_miniqmt.py" --mode backtest
...
class StrategyRunner:
    ...
    def __init__(self, dry_run=False):
        logger = get_logger()
        if logger is None:
            logger = FileLogger(STOCK_CODE, version='v22')
            set_logger(logger)
    ...
    def run(self):
        ...
        if not self.dry_run:
            ...
            if not self.conn.connect_data(): _log('[ERROR] market data connect failed'); return
        self._init_state()
        _log('[START] {} v22 {} {}'.format(STOCK_NAME, 'LIVE' if not self.dry_run else 'SIGNAL', STOCK_QMT))
        ...
        try:
            ...
            _log('[STOP] {} v22 cum {} days gross~Y{:,.0f}'.format(STOCK_NAME, self.total_t_days, self.total_pnl))
            logger = get_logger()
...
def main():
    parser = argparse.ArgumentParser(description='MiniQMT 迷你反T v22 — 阶梯加仓机制')
    ...
    logger = FileLogger(STOCK_CODE, version='v22'); set_logger(logger)
