...
"""
================================================================================
 MiniQMT — QMT day trading v27 + 阶梯加仓机制 + 严格成交判定 + 短线动量反转
================================================================================
 ...
 [run mode]
 python "Stragety/MiniQMT_Stragety/DayTradeing_v27_stragety_miniqmt.py" --mode signal
 python "Stragety/MiniQMT_Stragety/DayTradeing_v27_stragety_miniqmt.py" --mode live
 python "Stragety/MiniQMT_Stragety/DayTradeing_v27_stragety_miniqmt.py" --mode backtest
...
class StrategyRunner:
    """MiniQMT v27 — 阶梯加仓 + 严格成交判定 + 下单前仓位检查 + 短线动量反转(自适应ATR)"""

    def __init__(self, dry_run=False):
        logger = get_logger()
        if logger is None:
            logger = FileLogger(STOCK_CODE, version='v27')
            set_logger(logger)
    ...
    def run(self):
        ...
        if not self.dry_run:
            ...
            if not self.conn.connect_data(): _log('[ERROR] market data connect failed'); return
        self._init_state()
        _log('[START] {} v27 {} {}'.format(STOCK_NAME, 'LIVE' if not self.dry_run else 'SIGNAL', STOCK_QMT))
        ...
        try:
            ...
            _log('[STOP] {} v27 cum {} days gross~Y{:,.0f}'.format(STOCK_NAME, self.total_t_days, self.total_pnl))
            logger = get_logger()
...
def main():
    parser = argparse.ArgumentParser(description='MiniQMT internal daily Trading v27 — 严格成交判定 + 下单前仓位检查 + 短线动量反转(自适应ATR)')
    ...
    logger = FileLogger(STOCK_CODE, version='v27'); set_logger(logger)
