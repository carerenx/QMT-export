...
"""
================================================================================
 MiniQMT — QMT day trading v31 + FIX限价买回 + 严格成交判定
================================================================================
 ...
 python "Stragety/MiniQMT_Stragety/DayTradeing_v31_stragety_miniqmt.py" --mode live
 python "Stragety/MiniQMT_Stragety/DayTradeing_v31_stragety_miniqmt.py" --mode backtest
...
class StrategyRunner:
    """MiniQMT v31 — MOM/REV-T FIX限价买回 + 严格成交判定。"""

    def __init__(self, dry_run=False):
        logger = get_logger()
        if logger is None:
            logger = FileLogger(STOCK_CODE, version='v31')
            set_logger(logger)
    ...
    def run(self):
        ...
        if not self.dry_run:
            ...
            if not self.conn.connect_data(): _log('[ERROR] market data connect failed'); return
        self._init_state()
        _log('[START] {} v31 {} {}'.format(STOCK_NAME, 'LIVE' if not self.dry_run else 'SIGNAL', STOCK_QMT))
        ...
        try:
            ...
            _log('[STOP] {} v31 cum {} days gross~Y{:,.0f}'.format(STOCK_NAME, self.total_t_days, self.total_pnl))
            logger = get_logger()
...
def main():
    parser = argparse.ArgumentParser(description='MiniQMT internal daily Trading v31 — MOM/REV-T FIX限价买回')
    ...
    logger = FileLogger(STOCK_CODE, version='v31'); set_logger(logger)
