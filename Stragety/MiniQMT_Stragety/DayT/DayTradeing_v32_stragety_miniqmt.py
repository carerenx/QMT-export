...
"""
================================================================================
 MiniQMT — QMT day trading v32 + MOM触发阈值优化 + FIX限价买回
================================================================================
 ...
 [run mode]
 python "Stragety/MiniQMT_Stragety/DayTradeing_v32_stragety_miniqmt.py" --mode signal
 python "Stragety/MiniQMT_Stragety/DayTradeing_v32_stragety_miniqmt.py" --mode live
 python "Stragety/MiniQMT_Stragety/DayTradeing_v32_stragety_miniqmt.py" --mode backtest
...
MOM_ATR_MULT          = 4.0           # 触发幅度 = 2 × 最近10分钟ATR (自适应)
...
class StrategyRunner:
    """MiniQMT v32 — MOM触发阈值优化 + MOM/REV-T FIX限价买回。"""

    def __init__(self, dry_run=False):
        logger = get_logger()
        if logger is None:
            logger = FileLogger(STOCK_CODE, version='v32')
            set_logger(logger)
    ...
    def run(self):
        ...
        if not self.dry_run:
            ...
            if not self.conn.connect_data(): _log('[ERROR] market data connect failed'); return
        self._init_state()
        _log('[START] {} v32 {} {}'.format(STOCK_NAME, 'LIVE' if not self.dry_run else 'SIGNAL', STOCK_QMT))
        ...
        try:
            ...
            _log('[STOP] {} v32 cum {} days gross~Y{:,.0f}'.format(STOCK_NAME, self.total_t_days, self.total_pnl))
            logger = get_logger()
...
def main():
    parser = argparse.ArgumentParser(description='MiniQMT internal daily Trading v32 — MOM触发阈值优化 + FIX限价买回')
    ...
    logger = FileLogger(STOCK_CODE, version='v32'); set_logger(logger)
