...
"""
================================================================================
 MiniQMT — QMT day trading v33 + MOM/REV-T让权 + MOM自适应回撤
================================================================================
 ...
 [run mode]
 python "Stragety/MiniQMT_Stragety/DayTradeing_v33_stragety_miniqmt.py" --mode signal
 python "Stragety/MiniQMT_Stragety/DayTradeing_v33_stragety_miniqmt.py" --mode live
 python "Stragety/MiniQMT_Stragety/DayTradeing_v33_stragety_miniqmt.py" --mode backtest
...
class StrategyRunner:
    """MiniQMT v33 — MOM/REV-T让权 + MOM自适应回撤 + FIX限价买回。"""

    def __init__(self, dry_run=False):
        logger = get_logger()
        if logger is None:
            logger = FileLogger(STOCK_CODE, version='v33')
            set_logger(logger)
    ...
    def _print_daily_brief(self, signal):
        ...
        if MOM_ENABLED:
            ...
            _log('[MOM-v33] REV让权{} band {:.2f}% | 自适应回撤{} {:.2f}~{:.2f}% ATR×{:.2f}'.format(
    ...
    def run(self):
        ...
        if not self.dry_run:
            ...
            if not self.conn.connect_data(): _log('[ERROR] market data connect failed'); return
        self._init_state()
        _log('[START] {} v33 {} {}'.format(STOCK_NAME, 'LIVE' if not self.dry_run else 'SIGNAL', STOCK_QMT))
        ...
        try:
            ...
            _log('[STOP] {} v33 cum {} days gross~Y{:,.0f}'.format(STOCK_NAME, self.total_t_days, self.total_pnl))
            logger = get_logger()
...
def main():
    parser = argparse.ArgumentParser(description='MiniQMT internal daily Trading v33 — MOM/REV-T让权 + MOM自适应回撤')
    ...
    logger = FileLogger(STOCK_CODE, version='v33'); set_logger(logger)
