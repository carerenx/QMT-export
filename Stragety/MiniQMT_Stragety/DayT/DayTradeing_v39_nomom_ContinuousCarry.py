...
class StrategyRunner:
    ...
    def _capture_overnight_carry(self):
        """Return one attributable open leg that must survive the daily reset."""
        st = self.st
        short_legs = list(st.get('short_legs', []))
        long_legs = list(st.get('long_legs', []))
        mom_shares = int(st.get('mom_leg_shares', 0) or 0)
        kinds = int(bool(short_legs)) + int(bool(long_legs)) + int(mom_shares > 0)
        if not kinds:
            return None
        if kinds != 1:
            raise RuntimeError('overnight carry has multiple leg types; manual reconciliation required')
        if short_legs:
            return dict(kind='short', short_legs=short_legs,
                        fstate=STATE_SOLD,
                        sell_fill_price=st.get('sell_fill_price', 0.0),
                        buyback_target=st.get('buyback_target', 0.0),
                        buyback_target_pct=st.get('buyback_target_pct', 0.0),
                        dip_price=st.get('dip_price', 0.0),
                        ladder_sell_target=st.get('ladder_sell_target', 0.0),
                        ladder_sold_count=st.get('ladder_sold_count', 0))
        if long_legs:
            return dict(kind='long', long_legs=long_legs,
                        fstate=STATE_BT_BOUGHT,
                        bt_buy_fill_price=st.get('bt_buy_fill_price', 0.0),
                        bt_sellback_target=st.get('bt_sellback_target', 0.0),
                        bt_sell_peak_price=st.get('bt_sell_peak_price', 0.0),
                        ladder_buy_target=st.get('ladder_buy_target', 0.0),
                        ladder_bought_count=st.get('ladder_bought_count', 0))
        return dict(kind='mom', mom_state=st.get('mom_state', 'MOM_IDLE'),
                    mom_leg_shares=mom_shares,
                    mom_sell_price=st.get('mom_sell_price', 0.0),
                    mom_buy_price=st.get('mom_buy_price', 0.0),
                    mom_peak=st.get('mom_peak', 0.0), mom_dip=st.get('mom_dip', 0.0),
                    mom_cooldown_until=st.get('mom_cooldown_until', 0.0),
                    mom_cooldown_trigger=st.get('mom_cooldown_trigger', 0.0),
                    mom_cooldown_cycles=st.get('mom_cooldown_cycles', 0),
                    mom_cooldown_duration=st.get('mom_cooldown_duration', 0.0))

    def _restore_overnight_carry(self, carry):
        restored = dict(carry)
        restored.pop('kind', None)
        self.st.update(restored)
        self.st['overnight_carry_active'] = True
        self.st['state_enter_time'] = cfg.now_hms()
        _log('[OVERNIGHT-CARRY] {} leg restored; new entries blocked until flat'.format(carry['kind']))
    ...
    def _new_leg_block_reason(self):
        if self.st.get('overnight_carry_active', False):
            if not (self.st.get('short_legs') or self.st.get('long_legs') or
                    self.st.get('mom_leg_shares', 0)):
                self.st['overnight_carry_active'] = False
                _log('[OVERNIGHT-CARRY] flat; new entries re-enabled')
            else:
                return 'overnight carry awaiting reconciliation'
    ...
    def _daily_init(self):
        ...
        carry = self._capture_overnight_carry() if is_new_day else None
        ...
        self.st['locked'] = False; self.st['lock_reason'] = ''; self.st['lock_since'] = ''
        if carry:
            self._restore_overnight_carry(carry)
