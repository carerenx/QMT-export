...
# v40: restore missing strategic base inventory only after the original
# FWD-T dip-and-bounce entry signal confirms.
TARGET_BASE_SHARES = 200
BASE_RECOVERY_ENABLED = True
RECOVERY_MAX_ORDERS_PER_DAY = 3
STATE_RECOVERY_DIPPING = 'RECOVERY_DIPPING'
STATE_RECOVERY_DONE = 'RECOVERY_DONE'
...
class StrategyRunner:
    ...
    def __init__(self, dry_run=False):
        logger = get_logger()
        if logger is None:
            logger = FileLogger(STOCK_CODE, version='v40_base_recovery')
            set_logger(logger)
    ...
    def _daily_init(self):
        ...
        recovery_needed = max(0, TARGET_BASE_SHARES - int(base_shares or 0))
        recovery_needed = (
            recovery_needed // TRADE_LOT_SIZE * TRADE_LOT_SIZE)
        recovery_active = (
            BASE_RECOVERY_ENABLED and recovery_needed >= TRADE_LOT_SIZE)
        if recovery_active:
            do_short = False
            do_long = True
            short_reason = 'base recovery owns trading'
            long_reason = 'base recovery via FWD-T dip/bounce signal'
        ...
        self.st['recovery_active'] = recovery_active
        self.st['recovery_needed'] = recovery_needed
        self.st['recovery_dip_price'] = 0.0
        self.st['recovery_filled'] = 0
        self.st['recovery_order_count'] = 0
        self.st['recovery_done_today'] = False
    ...
    def _handle_idle(self, price):
        ...
        if st.get('recovery_active', False):
            buy_trigger = signal.get('buy_trigger', 0)
            if buy_trigger > 0 and price <= buy_trigger:
                st['fstate'] = STATE_RECOVERY_DIPPING
                st['recovery_dip_price'] = price
                st['state_enter_time'] = cfg.now_hms()
                _log('[BASE-RECOVERY dip] Y{:.2f} <= Y{:.2f}; wait bounce'.format(
                    price, buy_trigger))
            return
    ...
    def _handle_recovery_dipping(self, price):
        """Restore base shares on the original FWD-T bounce signal."""
        st = self.st
        dip = st.get('recovery_dip_price', price) or price
        if price < dip:
            dip = price
            st['recovery_dip_price'] = price
        bounce = (price - dip) / dip if dip > 0 else 0.0
        if bounce < cfg.BOUNCE_PCT:
            return

        self._refresh_position()
        current_shares = int(st.get('base_shares', 0) or 0)
        needed = max(0, TARGET_BASE_SHARES - current_shares)
        needed = needed // TRADE_LOT_SIZE * TRADE_LOT_SIZE
        st['recovery_needed'] = needed
        if needed < TRADE_LOT_SIZE:
            st['recovery_active'] = False
            st['recovery_done_today'] = True
            st['do_short'] = False
            st['do_long'] = False
            st['fstate'] = STATE_RECOVERY_DONE
            return

        _log('[BASE-RECOVERY buy trig] low Y{:.2f} bounce {:.2f}% -> Y{:.2f}, need {} sh'.format(
            dip, bounce * 100, price, needed))
        st['recovery_order_count'] = st.get('recovery_order_count', 0) + 1
        status, delta = self._submit_order(needed, price, 'FWD-T buy')
        bought = delta if delta > 0 else 0
        if bought <= 0:
            _log('[BASE-RECOVERY {}] no fill; retain recovery state'.format(status))
            if st['recovery_order_count'] >= RECOVERY_MAX_ORDERS_PER_DAY:
                st['do_short'] = False
                st['do_long'] = False
                st['fstate'] = STATE_RECOVERY_DONE
                _log('[BASE-RECOVERY PAUSED] daily order limit reached')
            return

        st['recovery_filled'] = st.get('recovery_filled', 0) + bought
        self._refresh_position()
        current_shares = int(st.get('base_shares', 0) or 0)
        remaining = max(0, TARGET_BASE_SHARES - current_shares)
        remaining = remaining // TRADE_LOT_SIZE * TRADE_LOT_SIZE
        st['recovery_needed'] = remaining
        if remaining >= TRADE_LOT_SIZE:
            st['recovery_dip_price'] = price
            _log('[BASE-RECOVERY PARTIAL] bought {} sh, remaining {} sh'.format(
                bought, remaining))
            return
        ...
        st['do_long'] = False
        st['fstate'] = STATE_RECOVERY_DONE
        _log('[BASE-RECOVERY DONE] bought {} sh; base restored to {} sh'.format(
            st.get('recovery_filled', 0), current_shares))
    ...
    def run(self):
        ...
        if not self.dry_run:
            ...
            if not self.conn.connect_data(): _log('[ERROR] market data connect failed'); return
        self._init_state()
        _log('[START] {} v40-base-recovery {} {}'.format(
        ...
        try:
            while self._running:
                ...
                elif fstate == STATE_RECOVERY_DIPPING: self._handle_recovery_dipping(price)
            ...
            _log('[STOP] {} v40-base-recovery cum {} days gross~Y{:,.0f}'.format(
...
def main():
    ...
    logger = FileLogger(STOCK_CODE, version='v40_base_recovery'); set_logger(logger)
