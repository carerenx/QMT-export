...
"""
================================================================================
 MiniQMT �� QMT day trading v40 + ���T�󰴵��վ������㴥����
================================================================================

 [v40 �Ķ�] (vs v39)
   - ÿ���һ������T������ʱ�ۼƳɽ���/ԭʼ�ɽ������㵱�ճɽ�����
   - ��һ��REV-T/FWD-T�������Ըþ���Ϊ��׼���¼��㲢�̶�����һ��
   - ���վ���������Чʱ�������д����ۣ�����������鵼����ֵƯ��
...
class StrategyRunner:
    ...
    def _print_daily_brief(self, signal):
        ...
        if MOM_ENABLED:
            ...
            _log('[MOM-v40] REV��Ȩ{} band {:.2f}% (������REV��ֵ) | ����Ӧ�س�{} {:.2f}~{:.2f}% ATR��{:.2f}'.format(
    ...
    def _force_buyback(self):
        ...
        if bought >= shares:
            ...
            st['fstate'] = STATE_SOLD
            _log('[REV-T force PARTIAL] ����� {} sh, ʣ�� {} sh'.format(
                bought, shares - bought))
        else:
            st['fstate'] = STATE_SOLD
            _log('[WARN] REV-T force buyback δ�ɽ�, ���ּ��')
    ...
    def _do_bt_force_sell(self):
        ...
        if sold >= shares:
            ...
            st['fstate'] = STATE_BT_BOUGHT
            _log('[FWD-T force PARTIAL] ������ {} sh, ʣ�� {} sh'.format(
                sold, shares - sold))
        else:
            st['fstate'] = STATE_BT_BOUGHT
            _log('[WARN] FWD-T force sell δ�ɽ�, ���ּ��')
        ...
        if MOM_EMERGENCY_BUYBACK_ENABLED and price >= sp * (1.0 + cfg.EMERGENCY_BUYBACK_PCT):
            ...
            if bought > 0:
                ...
                if bought >= shares:
                    ...
                    st['mom_state'] = 'MOM_SOLD'
                    _log('[MOM EMERG buyback PARTIAL] ����� {} sh, ʣ�� {} sh'.format(
                        bought, shares - bought))
            return
        # ������ش���: ��1.2% �� ̽�׻������
    ...
    def _mom_handle_buyback_cooling(self, price, now_ts=None):
        ...
        if price > trigger and price < profit_ceiling:
            ...
            if bought < shares:
                ...
                _log('[MOM buyback PARTIAL] ����� {} sh, ʣ�� {} sh'.format(
                    bought, shares - bought))
                return
            self.total_t_days += 1
        ...
        if price <= bp * (1.0 - cfg.STOP_LOSS_PCT):
            ...
            if sold > 0:
                ...
                if sold < shares:
                    ...
                    st['mom_state'] = 'MOM_BT_BOUGHT'
                    _log('[MOM stop-loss PARTIAL] ������ {} sh, ʣ�� {} sh'.format(
                        sold, shares - sold))
                    return
                self.total_t_days += 1
    ...
    def _mom_handle_sellback_cooling(self, price, now_ts=None):
        ...
        if price < trigger and price > profit_floor:
            ...
            if sold < shares:
                ...
                st['mom_state'] = 'MOM_BT_BOUGHT'
                self._mom_clear_close_cooldown()
                _log('[MOM sellback PARTIAL] ������ {} sh, ʣ�� {} sh'.format(
                    sold, shares - sold))
                return
            self.total_t_days += 1
    ...
    def _mom_force_close(self, price):
        ...
        if ms in ('MOM_SOLD', MOM_STATE_BUYBACK_COOLING):
            ...
            if bought >= shares:
                ...
                st['mom_leg_shares'] = shares - bought
                st['mom_state'] = 'MOM_SOLD'
                _log('[MOM force buyback PARTIAL] ����� {} sh, ʣ�� {} sh'.format(
                    bought, shares - bought))
            else:
            ...
            if sold >= shares:
                ...
                st['mom_leg_shares'] = shares - sold
                st['mom_state'] = 'MOM_BT_BOUGHT'
                _log('[MOM force sell PARTIAL] ������ {} sh, ʣ�� {} sh'.format(
                    sold, shares - sold))
            else:
...
def main():
    parser = argparse.ArgumentParser(description='MiniQMT internal daily Trading v40 �� ���T�󰴵��վ���������һ�ִ�����')
