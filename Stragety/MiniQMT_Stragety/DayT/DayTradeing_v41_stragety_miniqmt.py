...
"""
================================================================================
 MiniQMT — QMT day trading v41：独立策略基线 + 10 分钟 MA 仓位报告
================================================================================

 [v41 改动] (vs v40)
   - 以 v40 完整策略代码为基线独立构建；不导入或继承任何历史策略文件。
   - 每 10 分钟用“最近已完成日线 + 当前价”计算 MA5/MA20，并输出仓位位置与 MA20 风险线。
   - 已确认行情 tick 的昨收滞后一日时，使用日线快照校验后的未复权昨收。

 [run mode]
 python "Stragety/MiniQMT_Stragety/DayTradeing_v41_stragety_miniqmt.py" --mode signal
 python "Stragety/MiniQMT_Stragety/DayTradeing_v41_stragety_miniqmt.py" --mode live
 python "Stragety/MiniQMT_Stragety/DayTradeing_v41_stragety_miniqmt.py" --mode backtest
...
class StrategyRunner:
    """MiniQMT v41 — 完成T后按当日均价重算下一轮触发价。"""

    def __init__(self, dry_run=False):
        logger = get_logger()
        if logger is None:
            logger = FileLogger(STOCK_CODE, version='v41')
            set_logger(logger)
        self.version = 'v41'
    ...
    def _init_state(self):
        self.st.update({
            ...
            # v41: previous completed closes plus the current intraday price form MA5/MA20.
    ...
    def _update_intraday_average(self, tick_data):
        """Cache VWAP and report v41 intraday MA position every ten minutes."""
    ...
    def run(self):
        ...
        try:
            ...
            _log('[STOP] {} v41 cum {} days gross~Y{:,.0f}'.format(STOCK_NAME, self.total_t_days, self.total_pnl))
            logger = get_logger()
...
def main():
    parser = argparse.ArgumentParser(description='MiniQMT internal daily Trading v41 - 10-minute MA report — 完成T后按当日均价重算下一轮触发价')
    ...
    logger = FileLogger(STOCK_CODE, version='v41'); set_logger(logger)
