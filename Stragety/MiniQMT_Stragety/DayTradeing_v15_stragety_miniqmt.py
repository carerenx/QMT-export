...
"""
================================================================================
 MiniQMT — QMT day trading v15 + 模块化拆分
================================================================================
 VS Code connect xtquant MiniQMT run v15 strategy。
 ...
 [run mode]
   mode1 — signal mode:
     python "Stragety/MiniQMT_Stragety/DayTradeing_v15_stragety_miniqmt.py" --mode signal
   mode2 — real mode:
     python "Stragety/MiniQMT_Stragety/DayTradeing_v15_stragety_miniqmt.py" --mode live
   mode3 — backtest mode:
     python "Stragety/MiniQMT_Stragety/DayTradeing_v15_stragety_miniqmt.py" --mode backtest
...
class StrategyRunner:
    ...
    def _print_signal(self, signal):
        ...
        base_can_use = self.st['base_can_use']
        ...
        monitor_mode = ' + '.join([p for p in plan_parts if '不可用' not in p]) or '今日无可用交易方向'

        _log('')
        _log('╔' + '═' * 55 + '╗')
        _log('║  {}  迷你反T v15  |  {}  |  {}  ║'.format(STOCK_NAME, trade_date, trend))
        _log('╠' + '═' * 55 + '╣')
        ...
        if do_short:
            ...
            _log('  │  卖出确认: 回落 >= {:.2f}% -> 执行卖出'.format(cfg.PULLBACK_PCT * 100))
            _log('  │  买回触发:  卖价 x (1 - ATR% x {:.2f})'.format(cfg.BUYBACK_TRIGGER_MULT))
            _log('  │  买回确认:  回升 >= {:.2f}% -> 执行买回'.format(cfg.BOUNCE_PCT * 100))
            _log('  │  紧急买回: +{:.0f}% | 止损: {:.1f}% | 尾盘: {}'.format(
                cfg.EMERGENCY_BUYBACK_PCT * 100, cfg.STOP_LOSS_PCT * 100, cfg.FORCE_CLOSE_TIME))
        ...
        if do_long:
            ...
            _log('  │     底线: Y{:.2f} (开盘{:.2f} - {:.1f}%)'.format(
                signal.get('buy_trigger_floor', 0), open_p, cfg.BUY_TRIGGER_PCT * 100))
            _log('  │     跟随: Y{:.2f} (当前价 - {:.1f}%)  ← 实时更新'.format(
                signal.get('buy_trigger_trail', 0), cfg.BUY_TRIGGER_TRAIL * 100))
            _log('  │     取较高者 -> 价格涨了触发线跟着涨')
            _log('  │  买入确认: 回升 >= {:.2f}% -> 执行买入'.format(cfg.BOUNCE_PCT * 100))
            _log('  │  ★ 卖出触发线:   Y{:.2f} (买价 + {:.1f}%)'.format(sell_hint, cfg.SELLBACK_RISE_PCT * 100))
            _log('  │  卖出确认: 回落 >= {:.2f}% -> 执行卖出'.format(cfg.PULLBACK_PCT * 100))
    ...
    def run(self):
        ...
        self._init_state()
        _log(f'{STOCK_NAME} 迷你反T v15 MiniQMT版 启动')
        ...
        try:
            ...
            _log(f'{STOCK_NAME} v15 MiniQMT版 已停止 | 累计 {self.total_t_days}天 | 毛利~Y{self.total_pnl:,.0f}')
...
def run_backtest_mode(start='20250801', end='20260806'):
    ...
    print(f'\n{"="*55}')
    print(f'  回测mode — QMT 迷你反T v15 (模块化)')
...
def main():
    parser = argparse.ArgumentParser(
        description='MiniQMT 迷你反T v15 策略 — 模块化拆分版',
...
v15 模块结构:
  core/config.py         → 策略参数常量
  core/indicators.py     → 技术指标计算
  core/signals.py        → 动态乘数 + 信号计算
  infra/logger.py        → 文件日志系统
  infra/connector.py     → MiniQMT连接 + QMT接口模拟
  ★ 本文件              → 只含 StrategyRunner 状态机 + CLI
