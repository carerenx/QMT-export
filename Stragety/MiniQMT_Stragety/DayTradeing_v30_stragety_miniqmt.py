...
"""
================================================================================
 MiniQMT — QMT day trading v30 + 均线位置/成交量/价格监控
================================================================================

 [v30 改动] (vs v29)
   ★ 行情位置监控 — 新增 MARKET_STATUS_PRINT_ENABLED 总开关，每60秒打印:
     · 当前价格与当日累计成交量
     · 当前价格相对动态MA5/MA20的高低位置、绝对价差及百分比
     · 当前价格低于MA20的97%时打印风险提示
 ...
 [run mode]
 python "Stragety/MiniQMT_Stragety/DayTradeing_v30_stragety_miniqmt.py" --mode signal
 python "Stragety/MiniQMT_Stragety/DayTradeing_v30_stragety_miniqmt.py" --mode live
 python "Stragety/MiniQMT_Stragety/DayTradeing_v30_stragety_miniqmt.py" --mode backtest
...
# ★ v30: 行情位置监控总开关
MARKET_STATUS_PRINT_ENABLED = True     # True=打印价格/成交量/MA位置及风险提示
MARKET_STATUS_PRINT_INTERVAL_SEC = 60  # 打印间隔（秒）
MA20_RISK_RATIO = 0.97                 # 现价低于MA20的97%时提示风险


class StrategyRunner:
    """MiniQMT v30 — v29交易逻辑 + 行情位置监控。"""

    def __init__(self, dry_run=False):
        logger = get_logger()
        if logger is None:
            logger = FileLogger(STOCK_CODE, version='v30')
            set_logger(logger)
        ...
        self._last_market_status_log = 0.0
        self._running = True
        self.total_t_days = 0
        self.total_pnl = 0.0
    ...
    def _init_state(self):
        self.st.update({
            ...
            # ★ v30: 用已完成交易日收盘价 + 当前价计算动态MA5/MA20
            'ma_completed_closes': [],
        })
    ...
    def _daily_init(self):
        ...
        self.st['ma_completed_closes'] = self._completed_closes_for_ma(
            hist_close[STOCK_QMT], last_close)
    ...
    @staticmethod
    def _completed_closes_for_ma(hist_closes, last_close):
        """整理已完成交易日收盘价，排除可能存在的今日未收盘日K。"""
        closes = []
        ...
        if len(closes) >= 2 and last_close and last_close > 0:
            # 本地日线若已包含今日未完成bar，倒数第二项通常等于tick昨收。
            # 优先识别并剔除该bar，避免把今日价格重复计入动态均线。
            prev_gap = abs(closes[-2] - last_close) / last_close
            last_gap = abs(closes[-1] - last_close) / last_close
            if prev_gap <= 0.001 and prev_gap <= last_gap:
                closes = closes[:-1]
        return closes

    @staticmethod
    def _ma_position_text(price, ma_value, ma_name):
        diff = price - ma_value
        pct = diff / ma_value * 100 if ma_value > 0 else 0.0
        relation = '高于' if diff >= 0 else '低于'
        return '{} Y{:.2f}: {} Y{:.2f} ({:+.2f}%)'.format(
            ma_name, ma_value, relation, abs(diff), pct)

    def _print_market_status(self, tick_data):
        """打印现价、成交量、动态MA位置；跌破MA20的97%时提示风险。"""
        ...
        if len(closes) < 19:
            _log('[行情] 现价 Y{:.2f} | 成交量 {:,.0f} | 均线数据不足({}/19)'.format(
                price, volume, len(closes)))
            return

        ma5 = float(np.mean(closes[-4:] + [price]))
        ma20 = float(np.mean(closes[-19:] + [price]))
        _log('[行情] 现价 Y{:.2f} | 成交量 {:,.0f} | {} | {}'.format(
            price, volume,
            self._ma_position_text(price, ma5, 'MA5'),
            self._ma_position_text(price, ma20, 'MA20')))

        risk_price = ma20 * MA20_RISK_RATIO
        if price < risk_price:
            _log('[风险] 当前价 Y{:.2f} 低于 MA20×97% Y{:.2f}，请注意下行风险'.format(
                price, risk_price))

    def _maybe_print_market_status(self, tick_data, now_ts):
        if not MARKET_STATUS_PRINT_ENABLED:
            return
        if now_ts - self._last_market_status_log < MARKET_STATUS_PRINT_INTERVAL_SEC:
            return
        self._last_market_status_log = now_ts
        self._print_market_status(tick_data)
    ...
    def run(self):
        ...
        if not self.dry_run:
            ...
            if not self.conn.connect_data(): _log('[ERROR] market data connect failed'); return
        self._init_state()
        _log('[START] {} v30 {} {}'.format(STOCK_NAME, 'LIVE' if not self.dry_run else 'SIGNAL', STOCK_QMT))
        ...
        try:
            ...
            _log('[STOP] {} v30 cum {} days gross~Y{:,.0f}'.format(STOCK_NAME, self.total_t_days, self.total_pnl))
            logger = get_logger()
...
def main():
    parser = argparse.ArgumentParser(description='MiniQMT internal daily Trading v30 — v29交易逻辑 + 行情位置监控')
    ...
    logger = FileLogger(STOCK_CODE, version='v30'); set_logger(logger)
