...
"""
================================================================================
 MiniQMT — QMT day trading v35 + REV-T卖价99%二次回升买回
================================================================================
 ...
 [run mode]
 python "Stragety/MiniQMT_Stragety/DayTradeing_v35_stragety_miniqmt.py" --mode signal
 python "Stragety/MiniQMT_Stragety/DayTradeing_v35_stragety_miniqmt.py" --mode live
 python "Stragety/MiniQMT_Stragety/DayTradeing_v35_stragety_miniqmt.py" --mode backtest
...
class StrategyRunner:
    """MiniQMT v35 — REV-T卖价99%二次回升买回 + v34全部机制。"""

    def __init__(self, dry_run=False):
        logger = get_logger()
        if logger is None:
            logger = FileLogger(STOCK_CODE, version='v35')
            set_logger(logger)
    ...
    def _mom_handle_bt_spiking(self, price):
        ...
        if pullback >= pullback_threshold:
            shares = st.get('mom_leg_shares', 0) or MOM_LOT_SIZE
            _log('[MOM sellback sell] peak Y{:.2f} 回落{:.2f}% >= {:.2f}% → Y{:.2f}'.format(
                peak, pullback * 100, pullback_threshold * 100, price))
    ...
    def run(self):
        ...
        if not self.dry_run:
            ...
            if not self.conn.connect_data(): _log('[ERROR] market data connect failed'); return
        self._init_state()
        _log('[START] {} v35 {} {}'.format(STOCK_NAME, 'LIVE' if not self.dry_run else 'SIGNAL', STOCK_QMT))
        ...
        try:
            ...
            _log('[STOP] {} v35 cum {} days gross~Y{:,.0f}'.format(STOCK_NAME, self.total_t_days, self.total_pnl))
            logger = get_logger()
...
def main():
    parser = argparse.ArgumentParser(description='MiniQMT internal daily Trading v35 — REV-T卖价99%二次回升买回')
