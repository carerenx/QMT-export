...
"""
================================================================================
 MiniQMT — QMT day trading v36 + MOM平仓前12秒冷却冻结
================================================================================
 ...
 [run mode]
 python "Stragety/MiniQMT_Stragety/DayTradeing_v36_stragety_miniqmt.py" --mode signal
 python "Stragety/MiniQMT_Stragety/DayTradeing_v36_stragety_miniqmt.py" --mode live
 python "Stragety/MiniQMT_Stragety/DayTradeing_v36_stragety_miniqmt.py" --mode backtest
...
# ★ v36: MOM平仓确认冷却（非阻塞，由tick时间戳驱动）
MOM_CLOSE_COOLDOWN_SEC = 12.0
MOM_CLOSE_MIN_PROFIT_PCT = 0.005



class StrategyRunner:
    """MiniQMT v36 — MOM平仓前12秒冷却冻结 + v35全部机制。"""

    def __init__(self, dry_run=False):
        logger = get_logger()
        if logger is None:
            logger = FileLogger(STOCK_CODE, version='v36')
            set_logger(logger)
    ...
    def _daily_init(self):
        ...
        _log('[VOLUME-DATA] count={} tail={}'.format(
    ...
    def _mom_status(self):
        ...
        if ms == MOM_STATE_BUYBACK_COOLING:
            return '买回冷却: trigger Y{:.2f} cycle {}'.format(
        ...
        if ms == MOM_STATE_SELLBACK_COOLING:
            return '卖回冷却: trigger Y{:.2f} cycle {}'.format(
                st.get('mom_cooldown_trigger', 0.0), st.get('mom_cooldown_cycles', 0))
    ...
    def _mom_start_close_cooldown(self, state, trigger, price, now_ts, label):
        ...
        st['mom_cooldown_until'] = now + MOM_CLOSE_COOLDOWN_SEC
        st['mom_cooldown_cycles'] = st.get('mom_cooldown_cycles', 0) + 1
        _log('[MOM {} cooldown #{}] Y{:.2f}, freeze {:.0f}s until {:.3f}'.format(
            label, st['mom_cooldown_cycles'], price,
            MOM_CLOSE_COOLDOWN_SEC, st['mom_cooldown_until']))
    ...
    def _mom_handle_buyback_cooling(self, price, now_ts=None):
        ...
        if price > trigger and price < profit_ceiling:
            ...
            if delta <= 0:
                _log('[MOM buyback FAIL] 未成交, 续冻12s')
                self._mom_start_close_cooldown(
    ...
    def _mom_handle_sellback_cooling(self, price, now_ts=None):
        ...
        if price < trigger and price > profit_floor:
            ...
            sold = -delta
            if sold <= 0:
                _log('[MOM sellback FAIL] 未成交, 续冻12s')
                self._mom_start_close_cooldown(
    ...
    def run(self):
        ...
        if not self.dry_run:
            ...
            if not self.conn.connect_data(): _log('[ERROR] market data connect failed'); return
        self._init_state()
        _log('[START] {} v36 {} {}'.format(STOCK_NAME, 'LIVE' if not self.dry_run else 'SIGNAL', STOCK_QMT))
        ...
        try:
            ...
            _log('[STOP] {} v36 cum {} days gross~Y{:,.0f}'.format(STOCK_NAME, self.total_t_days, self.total_pnl))
            logger = get_logger()
...
def main():
    parser = argparse.ArgumentParser(description='MiniQMT internal daily Trading v36 — MOM平仓前12秒冷却冻结')
    ...
    logger = FileLogger(STOCK_CODE, version='v34'); set_logger(logger)
