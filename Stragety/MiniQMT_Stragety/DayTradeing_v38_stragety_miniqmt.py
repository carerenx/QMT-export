...
"""
================================================================================
 MiniQMT — QMT day trading v38 + MOM冷却时间逐次递减
================================================================================
 ...
 [run mode]
 python "Stragety/MiniQMT_Stragety/DayTradeing_v38_stragety_miniqmt.py" --mode signal
 python "Stragety/MiniQMT_Stragety/DayTradeing_v38_stragety_miniqmt.py" --mode live
 python "Stragety/MiniQMT_Stragety/DayTradeing_v38_stragety_miniqmt.py" --mode backtest
...
# ★ v38: MOM平仓确认冷却（首次12秒，续冻每轮递减2秒，最低2秒）
...
class StrategyRunner:
    """MiniQMT v38 — MOM冷却时间逐次递减 + v36全部机制。"""

    def __init__(self, dry_run=False):
        logger = get_logger()
        if logger is None:
            logger = FileLogger(STOCK_CODE, version='v38')
            set_logger(logger)
    ...
    def _print_daily_brief(self, signal):
        ...
        if MOM_ENABLED:
            ...
            _log('[MOM-v34] REV让权{} band {:.2f}% (仅低于REV阈值) | 自适应回撤{} {:.2f}~{:.2f}% ATR×{:.2f}'.format(
    ...
    def run(self):
        ...
        if not self.dry_run:
            ...
            if not self.conn.connect_data(): _log('[ERROR] market data connect failed'); return
        self._init_state()
        _log('[START] {} v38 {} {}'.format(STOCK_NAME, 'LIVE' if not self.dry_run else 'SIGNAL', STOCK_QMT))
        ...
        try:
            while self._running:
                ...
                if fstate in (STATE_DONE, STATE_FORCED):
                    ...
                    if MOM_ENABLED and price > 0 and not self.st.get('locked', False):
                ...
                if MOM_ENABLED and not self.st.get('locked', False):
                    self._mom_tick(price, now_ts)
            ...
            _log('[STOP] {} v38 cum {} days gross~Y{:,.0f}'.format(STOCK_NAME, self.total_t_days, self.total_pnl))
            logger = get_logger()
...
def main():
    parser = argparse.ArgumentParser(description='MiniQMT internal daily Trading v38 — MOM冷却时间逐次递减')
    ...
    logger = FileLogger(STOCK_CODE, version='v38'); set_logger(logger)
