...
"""
================================================================================
 MiniQMT — QMT day trading v40 + 完成T后按当日均价重算触发价
================================================================================

 [v40 改动] (vs v39)
   - 每完成一笔完整T，按当时累计成交额/原始成交量计算当日成交均价
   - 下一笔REV-T/FWD-T触发价以该均价为基准重新计算并固定到下一轮
   - 当日均价数据无效时保留现有触发价，避免错误行情导致阈值漂移

 [v39 改动] (vs v38)
   - 零手不可执行：主策略与MOM建腿均要求至少1手可交易能力
   - MOM正T要求现金与可卖旧仓同时充足，避免买入后因T+1无法卖回
   - 启动即接近涨停时冻结所有新腿，开板并稳定后再解除
   - 修正ENABLED 0 lots、T+0和负数need等误导性日志

 [v38 改动] (vs v37)
   - 日线统一从 MiniQMT 当前活动数据目录读取，不再指定旧版 C:/QMT/datadir
   - 前复权序列用于指标，未复权收盘用于 tick lastClose 交叉校验
   - 剔除当日未完成日线并校验上一交易日；失败时锁定全部交易
   - 当日 tick open 独立参与触发价计算，不再覆盖历史 open 数组

 [v37 改动] (vs v36)
   - MOM首次平仓冷却12秒，后续每次续冻递减2秒
   - 冷却序列为12/10/8/6/4/2秒，达到2秒后保持最低2秒

 [v36 改动] (vs v35)
   - MOM冲高回落准备卖回时先冷却12秒；价格仍高于卖回触发价则续冻
   - 冷却后仅在“低于卖回触发价、但高于买入价0.5%”区间卖出
   - MOM探底回升准备买回时采用对称冷却；不满足有效价差区间则续冻

 [v35 改动] (vs v34)
   - 反T卖出后，价格首次下探至卖出价99%时锁存条件1
   - 若未触及原买回触发价而向上再次达到卖出价99%，立即按卖一价FIX买回

 [v34 改动] (vs v33)
   - MOM只在REV-T优先带内且低于卖出阈值时让权
   - 当前价达到或超过REV-T卖出阈值后，不再让权；已有让权状态立即释放

 [v33 改动] (vs v32)
   - MOM接近日线REV-T触发区时让权，避免同一波上涨重复卖出
   - MOM冲高回落使用独立、波动自适应的回撤阈值，不再强制共用主机制0.1%
   - 两项优化分别由独立布尔开关控制，关闭后恢复v32行为

 [v32 改动] (vs v31)
   - MOM_ATR_MULT 从3.5提高到4.0，降低短周期波动下的误触发频率
 ...
 [run mode]
 python "Stragety/MiniQMT_Stragety/DayTradeing_v40_stragety_miniqmt.py" --mode signal
 python "Stragety/MiniQMT_Stragety/DayTradeing_v40_stragety_miniqmt.py" --mode live
 python "Stragety/MiniQMT_Stragety/DayTradeing_v40_stragety_miniqmt.py" --mode backtest
...
# ★ v35: 反T卖出后先跌到卖价99%，未触及原买回价即反弹回99%时买回
...
def calculate_next_t_triggers(daily_average, atr_pct, sell_mult,
    ...
    if base <= 0:
        return None
    sell_raw = base * (
        1.0 + float(atr_pct or 0.0) * float(sell_mult or 0.0) *
        cfg.SELL_TRIGGER_SCALE)
    range_cap = base * (
        1.0 + float(daily_range_ma10 or 0.0) * cfg.DAILY_RANGE_CAP_MULT)
    range_capped = bool(
        cfg.DAILY_RANGE_CAP_ENABLED and range_cap > base and
        sell_raw > range_cap)
...
class StrategyRunner:
    """MiniQMT v40 — 完成T后按当日均价重算下一轮触发价。"""

    def __init__(self, dry_run=False):
        logger = get_logger()
        if logger is None:
            logger = FileLogger(STOCK_CODE, version='v40')
            set_logger(logger)
    ...
    def _update_intraday_average(self, tick_data):
        """Cache the latest valid intraday VWAP from the cumulative tick."""
    ...
    def run(self):
        ...
        if not self.dry_run:
            ...
            if not self.conn.connect_data(): _log('[ERROR] market data connect failed'); return
        self._init_state()
        _log('[START] {} v40 {} {}'.format(STOCK_NAME, 'LIVE' if not self.dry_run else 'SIGNAL', STOCK_QMT))
        ...
        try:
            ...
            _log('[STOP] {} v40 cum {} days gross~Y{:,.0f}'.format(STOCK_NAME, self.total_t_days, self.total_pnl))
            logger = get_logger()
...
def main():
    parser = argparse.ArgumentParser(description='MiniQMT internal daily Trading v40 — 完成T后按当日均价重算下一轮触发价')
    ...
    logger = FileLogger(STOCK_CODE, version='v40'); set_logger(logger)
