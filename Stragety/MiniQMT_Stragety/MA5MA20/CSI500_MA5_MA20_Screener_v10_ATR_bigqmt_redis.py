# -*- coding: utf-8 -*-
"""Independent CSI 500 MA5/MA20 screener v10 through the BigQMT Redis bridge.
...
Evidence: v13 ATR-only backtest 20250903-20260902; in-sample, not a guarantee.
...
def run_screen(api=None, today=None):
    ...
    _log("[STEP 1/4] discovering CSI 500 constituents...")
    codes, source = api.universe()
    if not codes:
        _log("[ERROR] unable to obtain CSI 500 constituents")
        return {"universe_count": 0, "history_valid_count": 0,
                "quote_valid_count": 0, "below_ma5": [], "above_ma20": [],
                "selected": [], "skipped": 0}
    _log("[UNIVERSE] CSI 500: {} stocks ({})".format(len(codes), source))
    ...
    _log("[STEP 4/4] calculating v10 MA5/MA20 conditions...")
    ...
    _print_result_section(
        "CSI 500: price < MA5 by {:.2f}% AND price > MA20 AND ATR14 >= {:.2f}%".format(
...
def main(argv=None):
    parser = argparse.ArgumentParser(description="CSI 500 MA5/MA20 screener v10 through BigQMT Redis")
    ...
    try:
        ...
        api = BigQmtRedisApi()
        api.verify()
        _log("[START] CSI 500 MA5/MA20 screener v10; Redis bridge; signal-only")
