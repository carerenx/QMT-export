...
class BuybackFixPricingTest(unittest.TestCase):
    ...
    def test_limit_uses_ask1_plus_slippage_rounded_up_to_cent(self):
        runner = self._runner([368.00, 368.10])

        self.assertAlmostEqual(368.19, runner._buyback_limit_price(367.50))

    def test_limit_falls_back_to_trigger_price_when_ask_is_missing(self):
        runner = self._runner([])

        self.assertAlmostEqual(367.69, runner._buyback_limit_price(367.50))
    ...
    def test_buyback_submits_explicit_fix_price(self):
        ...
        self.assertAlmostEqual(368.19, calls[0][3])

    def test_incident_cash_can_cover_fix_limit(self):
        runner = self._runner([368.00])
        runner._available_cash = lambda: 39001.0

        self.assertEqual(100, V31.StrategyRunner._clamp_buy_shares(runner, 100, 368.19))
