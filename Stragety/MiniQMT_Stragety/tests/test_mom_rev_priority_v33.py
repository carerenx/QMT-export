#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Regression tests for v33 MOM/REV-T priority and adaptive pullback."""
...
STRATEGY_PATH = os.path.join(STRATEGY_DIR, 'DayTradeing_v33_stragety_miniqmt.py')
SPEC = importlib.util.spec_from_file_location('daytrading_v33', STRATEGY_PATH)
V33 = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(V33)


class MomRevPriorityV33Test(unittest.TestCase):
    def setUp(self):
        self.original_priority = V33.MOM_REV_PRIORITY_ENABLED
        self.original_adaptive = V33.MOM_ADAPTIVE_PULLBACK_ENABLED

    def tearDown(self):
        V33.MOM_REV_PRIORITY_ENABLED = self.original_priority
        V33.MOM_ADAPTIVE_PULLBACK_ENABLED = self.original_adaptive

    def _runner(self, pullback_pct=0.0035):
        runner = V33.StrategyRunner.__new__(V33.StrategyRunner)
        runner.st = {
            'do_short': True,
            'daily_signal': {'sell_trigger': 364.82},
            'fstate': V33.STATE_IDLE,
    ...
    def test_adaptive_pullback_filters_incident_micro_pullback(self):
        V33.MOM_ADAPTIVE_PULLBACK_ENABLED = True
        V33.MOM_REV_PRIORITY_ENABLED = False
    ...
    def test_adaptive_pullback_uses_recent_minute_range_atr(self):
        ...
        runner.st['mom_atr_history'] = V33.deque([
            (0.0, 100.0), (1.0, 100.6),
    ...
    def test_adaptive_switch_off_restores_legacy_point_one_percent(self):
        V33.MOM_ADAPTIVE_PULLBACK_ENABLED = False
        V33.MOM_REV_PRIORITY_ENABLED = False
        runner = self._runner(pullback_pct=0.0035)
    ...
    def test_rev_priority_yields_instead_of_selling(self):
        V33.MOM_ADAPTIVE_PULLBACK_ENABLED = False
        V33.MOM_REV_PRIORITY_ENABLED = True
        runner = self._runner()

        runner._mom_handle_spiking(361.12)

        self.assertEqual([], runner.orders)
        self.assertEqual(V33.MOM_STATE_REV_YIELD, runner.st['mom_state'])
    ...
    def test_rev_keeps_priority_after_crossing_trigger_even_below_band(self):
        V33.MOM_REV_PRIORITY_ENABLED = True
        runner = self._runner()
        runner.st['fstate'] = V33.STATE_SPIKING
    ...
    def test_yield_state_releases_only_after_price_exits_zone(self):
        runner = self._runner()
        runner.st['mom_state'] = V33.MOM_STATE_REV_YIELD
