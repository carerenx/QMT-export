...
"""
================================================================================
 MiniQMT 适配版 — QMT 迷你反T v10 策略
================================================================================
 在 VS Code 中通过 xtquant 连接 MiniQMT 运行 v10 策略。

 【架构】
   VS Code (本脚本)
     ├─ xtdata     → QMT本地.DAT 历史数据 + 实时行情
     ├─ xttrader   → 交易下单 + 持仓/账户查询
     └─ v10策略逻辑 → 信号计算 + 状态机 (保持与QMT内置版完全一致)

 【前置条件】
   1. MiniQMT 已启动 (QMT → 右上角"极简模式")
   2. MiniQMT 中已登录资金账号 8890145315
   3. QMT 已下载 601869 历史日线数据
   4. pip install xtquant numpy pandas

 【运行方式】

   模式1 — 信号模式 (推荐首次运行, 只监控不下单):
     python "MyPy-Q/QMT_迷你反T_v10_miniqmt_实盘.py" --mode signal

   模式2 — 实盘模式 (自动下单):
     python "MyPy-Q/QMT_迷你反T_v10_miniqmt_实盘.py" --mode live

   模式3 — 回测模式 (用QMT本地数据回测, 不需要MiniQMT):
     python "MyPy-Q/QMT_迷你反T_v10_miniqmt_实盘.py" --mode backtest

 【与 QMT内置版 的区别】
   QMT内置:  策略在QMT客户端内运行, 使用 ContextInfo/handlebar 注入函数
   MiniQMT版: 策略在VS Code运行, 使用 xtdata/xttrader 直接调用
   策略逻辑:  完全一致, 参数完全一致
...
# ============================================================================
# 正T买入触发: 二选一(取较高者)
#   (1) 开盘价 × (1 - BUY_TRIGGER_PCT)    — 底线, 基于开盘
#   (2) 当前价 × (1 - BUY_TRIGGER_TRAIL)  — ★ 动态跟随, 价格涨了触发线也上移
#   效果: 开盘330时触发线=320, 涨到350时触发线=343, 只需回调2%即可触发
BUY_TRIGGER_PCT   = 0.03             # 买入触发底线: 开盘-3%
BUY_TRIGGER_TRAIL = 0.02             # ★ 动态跟随: 当前价-2% (价格涨触发线跟涨)
...
class MiniQMTConnector:
    ...
    def connect_data(self):
        ...
        try:
            ...
            print('[提示] 请先启动 MiniQMT (QMT → 右上角"极简模式")')
            return False
    ...
    def order_stock(self, stock_code, shares, style, price=None):
        ...
        # 转换下单类型
        if style in ('COMPETE', 'MARKET'):
            price_type = xtconstant.LATEST_PRICE
            order_price = 0  # 最新价不需要指定价格
        else:
        ...
        try:
            ret = self.trader.order_stock(
                ...
                order_price,
                'mini反T_v10',
                f'迷你反T_{"买回" if order_type == xtconstant.STOCK_BUY else "卖出"}',
            )
...
class StrategyRunner:
    """
    MiniQMT 策略运行器 — 在 VS Code 中驱动 v10 策略的状态机。
    ...
    def _daily_init(self):
        ...
        if self.st.get('trade_date', '') and self.st['trade_date'] != today:
            _log(f'\n[新交易日] {self.st["trade_date"]} -> {today}')

        self._reset_daily()
        self.st['trade_date'] = today       # ★ _reset_daily之后设置, 避免被_init_state覆盖
        ...
        lot_cost = open_price * TRADE_LOT_SIZE * 1.005  # 含手续费缓冲

        # 反T(先卖后买): 需要可卖持仓 >= 1手
        do_short = signal['do_short'] and (base_can_use >= TRADE_LOT_SIZE)
        short_reason = ''
        if signal['do_short'] and base_can_use < TRADE_LOT_SIZE:
            short_reason = f'可用{base_can_use}股<1手(反T)'

        # 正T(先买后卖): 需要可用资金 >= 1手
        do_long = avail_cash >= lot_cost
        long_reason = ''
        if not do_long:
            long_reason = f'资金不足(需Y{lot_cost:,.0f}>Y{avail_cash:,.0f})'

        # 正T目标价计算 — ★ 动态跟随: 取 max(开盘底线, 当前价-2%)
        #   底线: 开盘×0.97 (防止价格暴跌时触发线过低)
        #   跟随: 当前价×0.98 (价格涨了触发线跟着涨, 只需2%回调即可触发)
        tick_now = self.ctx.get_full_tick([STOCK_QMT])
    ...
    def _print_signal(self, signal):
        ...
        _log('║  持仓: {}股 Y{:,.0f}  |  可用: Y{:,.0f}  |  总资产: Y{:,.0f}  ║'.format(
            base_shares, pos_value, avail_cash, total_val))
        _log('╠' + '═' * 55 + '╣')
    ...
    def _handle_idle(self, price):
        ...
        if st.get('do_short', False):
            ...
            if price >= trigger:
                ...
                if can_use < TRADE_LOT_SIZE:
                    ...
                    _log(f'[反T冲高] Y{price:.2f} >= Y{trigger:.2f} -> 进入冲高监控')
                return
        ...
        if st.get('do_long', False):
            ...
            if price <= buy_trigger:
                ...
                _log(f'[正T探底] Y{price:.2f} <= Y{buy_trigger:.2f}(-{under_pct:.2f}%) -> 等回升{BOUNCE_PCT*100:.2f}%')
                return
    ...
    def _handle_spiking(self, price):
        ...
        if pullback >= PULLBACK_PCT:
            ...
            _log(f'[假突破] Y{price:.2f} 跌回触发线下 Y{trigger:.2f}')
    ...
    def _handle_dipping(self, price):
        ...
        if bounce >= BOUNCE_PCT:
            ...
            _log(f'[假跌破] Y{price:.2f} 涨回买回线上 Y{bt:.2f}')
            st['fstate'] = STATE_SOLD
            st['dip_price'] = 0.0
    ...
    def _handle_bt_dipping(self, price):
        ...
        # 假跌破 -> 回到IDLE
        elif price > st.get('bt_buy_trigger', 999999):
            st['fstate'] = STATE_IDLE
            st['bt_dip_price'] = 0.0

    def _handle_bt_bought(self, price):
        """正T BOUGHT: 已买入, 等涨到卖回触发线"""
        st = self.st
        target = st.get('bt_sellback_target', 999999)

        if price >= target:
            rise = (price - st['bt_buy_fill_price']) / st['bt_buy_fill_price'] * 100
            st['fstate'] = STATE_BT_SPIKING
            st['bt_sell_peak_price'] = price
            _log(f'[正T卖回监控] 涨{rise:.2f}% -> Y{price:.2f} >= Y{target:.2f} | 等回落{PULLBACK_PCT*100:.2f}%')
    ...
    def _handle_bt_spiking(self, price):
        ...
        # 假突破 -> 回到BOUGHT
        elif price < st.get('bt_sellback_target', 999999):
            st['fstate'] = STATE_BT_BOUGHT
            st['bt_sell_peak_price'] = 0.0
    ...
    def run(self):
        ...
        _log(f'  模式: {"信号监控(不下单)" if self.dry_run else "实盘交易"}')
            ...
            while self._running:
                ...
                if now_ts - self._last_heartbeat >= 60:
                    ...
                    if fs == STATE_IDLE:
                        ...
                        if bp > 0:
                            chg = (price - bp) / bp * 100
                            _log('[心跳] {} | Y{:.2f} | 买Y{:.2f} 现{}{:.2f}% | 卖出线Y{:.2f}'.format(
                                fs, price, bp, '+' if chg >= 0 else '', chg, target))
