...
================================================================================
 ...
 【v5.0 核心改动 — 回调属性修复】
  ...
  v4已知问题:
    - order_callback 使用 m_dOrderPrice(不存在) → AttributeError 崩溃
    - deal_callback 使用 m_fCommission / m_fStampTax(不存在) → AttributeError 崩溃
    - 回调崩溃 → order_pending 永远不会被清除 → 补单无限循环

  v5修复:
    - ★ order_callback: m_dOrderPrice → m_dLimitPrice (正确属性名, 参见API附录3)
      m_nVolumeTotal → m_nVolumeTotalOriginal (最初委托量)
    - ★ deal_callback: m_fCommission → m_dComssion (正确属性名, API本身拼写有误)
      移除不存在的 m_fStampTax
    - ★ 两个回调加 try-except 保护: 回调异常不再导致补单状态混乱
    - ★ 回调异常计数: stop() 时报告
...
# ── ★ 核心参数(用户输入) ──
SELL_TRIGGER_PCT = 0.04            # 卖出触发: 涨超开盘价2%
...
def order_callback(ContextInfo, order):
    """委托状态变化回调 — v5: 正确属性名 + 异常保护"""
    try:
        sm = {50: '已报', 52: '部成', 53: '全成', 54: '部撤', 55: '已撤', 56: '废单'}
        status = order.m_nOrderStatus
        if status in sm:
            # m_dLimitPrice=委托价格, m_nVolumeTraded=已成交, m_nVolumeTotalOriginal=最初委托量
...
def deal_callback(ContextInfo, deal):
    """成交状态变化回调 — v5: 正确属性名 + 异常保护"""
    st = ContextInfo.st
    try:
        # ★ 先清除待成交标记(最关键的一步, 必须在任何可能崩溃的代码之前)
        st['order_pending'] = False
        st['order_retry_logged'] = False

        # m_nDirection: 文档标注"股票买卖永远是48", 需用实际测试确认
        # 常见: 1=买, 2=卖; m_dPrice=成交价; m_nVolume=成交量
        d = '买' if deal.m_nDirection == 1 else '卖'
        price = deal.m_dPrice
        vol = deal.m_nVolume
        amt = price * vol
        # m_dComssion=手续费(API属性名拼写即为单s, 非Commission)
        fee = getattr(deal, 'm_dComssion', 0)

        # PnL: 卖出=收入-费用, 买入=-(支出+费用)
...
def _print_startup(ContextInfo):
    ...
    _log(f'{"="*55}')
    _log(f'  {STOCK_NAME} 阈值双向T v5.0 — 稳定版')
