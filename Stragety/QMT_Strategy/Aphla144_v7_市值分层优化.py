#coding:gbk
"""
Alpha#144 — 流动性冲击择时策略 v7 (市值分层优化版)
================================================
基于微观结构因子的中盘股择时策略，专为 QMT 实盘/仿真/回测环境设计。
在 v6 基础上，用更大样本(211笔配对交易)重新校准各档配额。

【v6→v7 优化（配额再平衡，基于 log2 更大样本）】

  数据来源（2026-08-12，211笔配对交易 / 112只股票，流通市值=成交额/换手率）：
    微型<50亿      6笔   胜率16.7% 收益和 -2%   （连续两样本最差）
    小型50-150亿  102笔  胜率45.1% 收益和 +201% （密集区，稳）
    中型150-300亿  73笔  胜率56.2% 收益和 +312% （★最优：大赢家密集，开山/石大胜华/天味等）
    中大300-500亿  18笔  胜率50.0% 收益和 +2%   （★断崖：陆家嘴单票反复止损拖累）
    大型500-1000亿 10笔  胜率40.0% 收益和 +19%  （样本少，依赖恒逸石化/山东高速）

  v7 结论与对策：
    发现1: 150-300亿 是稳健的最优区间(胜率56%+收益312%，多笔+15%大赢家)
           → 对策: 中型档配额 3→6（核心加配）
    发现2: 300-500亿 收益断崖(收益和仅+2%，陆家嘴单票5次连亏拖累)
           → 对策: 中大档配额 3→2
    发现3: 微型<50亿 连续两样本最差(胜率16-20%)
           → 对策: 配额 2→1（保留观察，不剔除）
    发现4: 50-150亿 密集区(102笔)但单位收益不及150-300亿
           → 对策: 配额 5→4（让位给最优区间）

  v7 分层设计（单位：亿元）：
    档位1  微型   <50亿        → 1只   （最差，降配+流动性强化）
    档位2  小型   50~150亿     → 4只   （密集区，略降）
    档位3  中型   150~300亿    → 6只   （★最优区间，核心加配）
    档位4  中大   300~500亿    → 2只   （收益断崖，降配）
    档位5  大型   500~1000亿   → 2只   （样本少，保持低位）
    ─────────────────────────────────────
    合计 最多 15 只；每档独立配额，某档无候选则跳过（不跨档借名额）。

  与 v6 的区别：
    1. 配额从 2/5/3/3/2 调整为 1/4/6/2/2（150-300亿核心加配，
       300-500亿降配，微型降配）
    2. 档位边界、微型股流动性强化 均保持不变
...
#   依据 log2 大样本(211笔)：150-300亿最优(+312%)，300-500亿断崖(+2%)，
#   微型最差(-2%)。配额向 150-300亿 倾斜。
#   超过最后一个档位上界(>1000亿) 不入选
MARKET_CAP_TIERS = [
    (50,    1, '微型 <50亿'),      # ★v7★ 连续两样本最差，降配1
    (150,   4, '小型 50-150亿'),    # ★v7★ 密集区，略降
    (300,   6, '中型 150-300亿'),   # ★v7★ 最优区间，核心加配6
    (500,   2, '中大 300-500亿'),   # ★v7★ 收益断崖，降配2
    (1000,  2, '大型 500-1000亿'),  # ★v7★ 样本少，保持2
]
...
def init(ContextInfo):
    print("[init] Alpha#144 流动性冲击择时策略 v7 (市值分层优化版)")
    print("[init] 分层: %s" % " | ".join(
        "%s%d只" % (t[2], t[1]) for t in MARKET_CAP_TIERS))
    print("[init] v7优化: 150-300亿核心加配 | 300-500亿降配 | 微型降配1")
