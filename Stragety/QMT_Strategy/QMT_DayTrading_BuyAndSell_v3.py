...
"""
================================================================================
 QMT 迷你做T策略 v7.1 — 双方向版
================================================================================
 QMT运行时注入: get_trade_detail_data / order_shares / ContextInfo.*

 【v7.1 vs v7.0 改动】
   ★ 回测初始持仓: 首bar用close价下单买入(20%仓位), 让QMT引擎接管后续持仓
   ★ 回测交易模拟: 从OHLC直接判定, 不再用path+state-machine (4个点无法模拟bounce)
   ★ 实盘信号: 严格用bar OHLC, 不调用get_full_tick覆盖历史数据
================================================================================
"""
...
MIN_POSITION_LOTS = 1
MAX_DAILY_TRADES  = 3
BT_INIT_CASH      = 500000   # 回测初始资金
BT_POSITION_PCT   = 0.20     # 回测初始仓位比例(20%)
...
def compute_signal(opens, highs, lows, closes, volumes):
    ...
    if trend == 'strong_bull':
        do_short = False; reason = '强牛禁反T'
    elif curr_vr < VOLUME_FILTER_RATIO:
        do_short = False; reason = '缩量({:.2f})'.format(curr_vr)
    elif curr_rsi > RSI_OVERBOUGHT:
        do_short = False; reason = 'RSI超买({:.0f})'.format(curr_rsi)
...
def init(ContextInfo):
    ...
    ContextInfo.st = {
        ...
        'startup_printed': False, 'state_enter_time': '', 'sell_elapsed_bars': 0,
        ...
        '_bt_inited': False, '_bt_summary_printed': False,
    }
    ContextInfo.run_time("ontimer", TIMER_INTERVAL, "2019-10-14 13:20:00", "SH")
    is_live = ContextInfo.is_last_bar()
    if not is_live:
        _log('[回测] O:{} H:{} L:{} C:{} 初始资金:{:,.0f} 仓位:{}%'.format(
            '计算中', '', '', '', BT_INIT_CASH, int(BT_POSITION_PCT * 100)))
    _log('[启动] {} v7.1 {} {}'.format(STOCK_NAME, '实盘' if is_live else '回测', STOCK_QMT))
...
def handlebar(ContextInfo):
    ...
    if not is_live and base_shares < TRADE_LOT_SIZE and not st.get('_bt_inited'):
        ...
        avail = accounts[0].m_dAvailable if accounts else float(BT_INIT_CASH)
        if avail >= BT_INIT_CASH: avail = BT_INIT_CASH  # 回测默认资金

        buy_cash = avail * BT_POSITION_PCT
        shares = int(buy_cash / (bar_close * 1.001) / TRADE_LOT_SIZE) * TRADE_LOT_SIZE
        if shares < TRADE_LOT_SIZE:
            shares = TRADE_LOT_SIZE

        _log('[回测] 首bar买入 {}股 @{:.2f} = {:,.0f}'.format(shares, bar_close, shares * bar_close))
        try:
            order_shares(STOCK_QMT, shares, 'FIX', bar_close, ContextInfo, ACCOUNT)
        except Exception as e:
            _log('[回测] 下单失败: {}'.format(e))
    ...
    if base_shares < TRADE_LOT_SIZE:
        if is_live:
            _log('[警告] 底仓不足({}股)'.format(base_shares))
        return
    ...
    short_reason = ''
    if not signal['do_short']: short_reason = signal.get('blocked_reason', '')
    elif short_lots < MIN_POSITION_LOTS: short_reason = '可用{}股<{}手'.format(base_can_use, MIN_POSITION_LOTS)

    long_reason = ''
    if not do_long:
        r = []
        if long_lots_cash < MIN_POSITION_LOTS: r.append('资金不足')
    ...
    st['day_pnl'] = 0.0; st['stop_loss_hit'] = False
    st['state_enter_time'] = _now(); st['sell_elapsed_bars'] = 0
    ...
    st['lock_cooldown_until'] = 0.0; st['price_history'] = []

    # ------ 回测: 直接OHLC交易判定 ------
    if not is_live:
        _bt_trade_from_ohlc(ContextInfo, signal, bar_open, bar_high, bar_low, bar_close,
                            do_short, do_long, short_lots, long_lots)
        return

    # 实盘: 打印状态&信号
    _print_status(ContextInfo, curr_price, avail_cash, pos_value, total_val, bar_close)
...
def _bt_trade_from_ohlc(ContextInfo, signal, open_p, high_p, low_p, close_p,
                        do_short, do_long, short_lots, long_lots):
    """
    回测交易判定: 用OHLC直接判断是否可成交, 不模拟逐tick状态机。

    反T: high >= sell_trigger 说明盘中冲高了 → 按trigger价卖出
         如果 low <= buyback_target → 按买回价买回, 完成一轮
    正T: low <= buy_trigger 说明盘中探底了 → 按trigger价买入
         如果 high >= sellback_target → 按卖出价卖出, 完成一轮
    """
    st = ContextInfo.st
    ...
    if do_short:
        ...
        if high_p >= sell_trig:
            # 卖出: 取触发价和开盘价的较大者(接近高点)
            sell_price = max(open_p, sell_trig)
            buyback_pct = atr_pct * BUYBACK_TRIGGER_MULT
            buyback_target = round(sell_price * (1.0 - buyback_pct), 2)

            if low_p <= buyback_target:
                # 完美: 卖在高位, 买在低位
                buy_price = buyback_target
            elif close_p <= sell_price * (1.0 + EMERGENCY_BUYBACK_PCT):
                # 尾盘没跌回买回线 → 收盘价强制买回(不赚钱但恢复持仓)
                buy_price = close_p
            else:
                # 冲太高回不来 → 紧急买回(亏损)
                buy_price = close_p  # 按收盘价买回

            gross = (sell_price - buy_price) * TRADE_LOT_SIZE
            _log('[反T] sell {:.2f} buy {:.2f} 毛利 {:+,.0f}  | O:{:.2f} H:{:.2f} L:{:.2f} C:{:.2f}'.format(
                sell_price, buy_price, gross, open_p, high_p, low_p, close_p))

            _mini_sell(ContextInfo, sell_price)
            if buy_price > sell_price:
                _log('[反T亏损] 买回价{:.2f}>卖价{:.2f} | 尾盘强平'.format(buy_price, sell_price))
                _force_buyback(ContextInfo)
            else:
                _mini_buyback(ContextInfo, buy_price, '回测')
            st['total_t_days'] += 1
            st['total_pnl'] += gross
            traded = True
    ...
    if do_long and not traded:
        ...
        if low_p <= buy_trig:
            # 买入: 取触发价和收盘价的较小者(接近低点)
            buy_price = min(open_p, buy_trig, close_p)  # 保守: 取三者最低
            if buy_price > buy_trig: buy_price = buy_trig  # 不超过触发价
            sell_target = round(buy_price * (1.0 + SELLBACK_RISE_PCT), 2)

            if high_p >= sell_target:
                # 完美: 买在低位, 卖在高位
                sell_price = sell_target
            elif close_p <= buy_price * (1.0 - STOP_LOSS_PCT):
                # 止损
                sell_price = close_p
                _log('[正T止损] buy {:.2f} close {:.2f}(-{:.1f}%)'.format(
                    buy_price, close_p, (buy_price - close_p) / buy_price * 100))
            else:
                # 没有到达卖出目标 → 尾盘强制卖出
                sell_price = close_p

            gross = (sell_price - buy_price) * TRADE_LOT_SIZE
            _log('[正T] buy {:.2f} sell {:.2f} 毛利 {:+,.0f}  | O:{:.2f} H:{:.2f} L:{:.2f} C:{:.2f}'.format(
                buy_price, sell_price, gross, open_p, high_p, low_p, close_p))

            _mini_buy(ContextInfo, buy_price)
            if sell_price < buy_price * (1.0 - STOP_LOSS_PCT):
                _do_bt_force_sell(ContextInfo)
            else:
                _mini_sell_bt(ContextInfo, sell_price)
            st['total_t_days'] += 1
            st['total_pnl'] += gross
    ...
    if st['total_t_days'] > 0 and not st.get('_bt_summary_printed'):
        pass  # 每笔交易都有日志, 最后stop时打总
...
def ontimer(ContextInfo):
    ...
    if fstate == STATE_SOLD and not st.get('stop_loss_hit', False):
        ...
        if st.get('day_pnl', 0) < -loss_limit:
            _log('[止损] {:.0f} > {:.0f}'.format(st['day_pnl'], loss_limit))
...
def _handle_idle(ContextInfo, price):
    ...
    if st.get('do_short', False) and not st.get('locked', False):
        ...
        if price >= trigger:
            ...
            _log('[反T #{}/{}] {:.2f}>={:.2f}'.format(tc + 1, MAX_DAILY_TRADES, price, trigger))
            return
    if st.get('do_long', False):
        ...
        if price <= buy_trigger:
            ...
            _log('[正T #{}/{}] {:.2f}<={:.2f}'.format(tc + 1, MAX_DAILY_TRADES, price, buy_trigger))
...
def _handle_spiking(ContextInfo, price):
    ...
    if pullback >= PULLBACK_PCT:
        _log('[反T卖出] peak {:.2f} pb {:.2f}% → {:.2f}'.format(peak, pullback * 100, price))
...
def _handle_dipping(ContextInfo, price):
    ...
    if bounce >= BOUNCE_PCT:
        ...
        _log('[反T买回] dip {:.2f} bounce {:.2f}% → {:.2f} {:+,.0f}'.format(dip, bounce * 100, price, gross))
...
def _handle_bt_dipping(ContextInfo, price):
    ...
    if bounce >= BOUNCE_PCT:
        ...
        if avail < need:
            _log('[正T买失败] 资金不足'); st['fstate'] = STATE_IDLE; return
...
def _handle_bt_bought(ContextInfo, price):
    ...
    if bp > 0 and price <= bp * (1.0 - STOP_LOSS_PCT):
        _do_bt_force_sell(ContextInfo); return
    if price >= target:
...
def _mini_buyback(ContextInfo, price, reason=''):
    ...
    need = price * TRADE_LOT_SIZE * 1.001; avail = _cash(ContextInfo)
    if avail < need:
        _log('[买回失败] 资金不足 {:,.0f}>{:,.0f}'.format(need, avail)); return
    try:
...
def order_callback(ContextInfo, order):
    ...
    if order.m_nOrderStatus in sm:
        _log('[委托] {:.2f} {}/{} {}'.format(order.m_dOrderPrice, order.m_nVolumeTraded,
               order.m_nVolumeTotal, sm[order.m_nOrderStatus]))


def deal_callback(ContextInfo, deal):
    st = ContextInfo.st
    d = 'B' if deal.m_nDirection == 1 else 'S'
...
def _print_status(ContextInfo, curr_price, avail_cash, pos_value, total_val, bar_close):
    ...
    _log('持仓:{}股×{:.2f}={:,.0f} 浮{:+,.0f}({:+.1f}%) 现金{:,.0f} 总{:,.0f}'.format(
        st['base_shares'], curr_price, pos_value, unreal_pnl, unreal_pct, avail_cash, total_val))
    if st['total_t_days'] > 0:
        _log('累计:{}笔 {:+,.0f}'.format(st['total_t_days'], st['total_pnl']))


def _print_signal(ContextInfo):
    st = ContextInfo.st; s = st['daily_signal']
    _log('Trend:{} O:{:.2f} ATR:{:.1f}% RSI:{:.0f} VR:{:.2f} Mult:{:.2f}'.format(
        s['trend'], s['open_price'], s['atr_pct']*100, s['rsi'], s['vol_ratio'], s['sell_mult']))
    _log('Short:{} trig:{:.2f} | Long:{} trig:{:.2f} lots:{}'.format(
        'Y' if st['do_short'] else 'N({})'.format(s.get('short_reason','?')),
        s['sell_trigger'],
        'Y' if st['do_long'] else 'N({})'.format(st.get('long_reason','?')),
        s.get('buy_trigger', 0), st['long_lots']))
...
def stop(ContextInfo):
    ...
    if st:
        _log(''); _log('=' * 55)
        _log('  {} v7.1 策略停止  累计{}笔 毛利{:+,.0f}'.format(
            STOCK_NAME, st.get('total_t_days', 0), st.get('total_pnl', 0)))
        _log('=' * 55)
