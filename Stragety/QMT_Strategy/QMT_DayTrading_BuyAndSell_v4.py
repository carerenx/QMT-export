...
"""
================================================================================
 QMT 迷你做T策略 v7.2 — 双方向版 (修复回测分时重复 & FIX价格越界)
================================================================================
 QMT运行时注入: get_trade_detail_data / order_shares / ContextInfo.*

 【v7.2 改动】
   ★ 回测: handlebar按分钟触发 → 加 _bt_trade_date 日级去重, 每天只交易一次
   ★ 回测: 下单改 COMPETE(对手价), 避免FIX价在分钟bar上越界
   ★ 实盘: ontimer tick级驱动, 与回测逻辑分离
================================================================================
"""
...
def compute_signal(opens, highs, lows, closes, volumes):
    ...
    elif curr_rsi > RSI_OVERBOUGHT: do_short = False; reason = 'RSI超买'
...
def init(ContextInfo):
    ...
    ContextInfo.st = {
        ...
        '_bt_inited': False,
        '_bt_trade_date': '',     # ★ v7.2: 日级去重, 避免分钟bar重复交易
    }
    ContextInfo.run_time("ontimer", TIMER_INTERVAL, "2019-10-14 13:20:00", "SH")
    is_live = ContextInfo.is_last_bar()
    _log('[启动] {} v7.2 {} {}'.format(STOCK_NAME, '实盘' if is_live else '回测', STOCK_QMT))
...
def handlebar(ContextInfo):
    ...
    if not is_live and base_shares < TRADE_LOT_SIZE and not st.get('_bt_inited'):
        bar_close_v = closes[STOCK_QMT][-1]
        avail_v = accounts[0].m_dAvailable if accounts else float(BT_INIT_CASH)
        if avail_v >= BT_INIT_CASH: avail_v = BT_INIT_CASH
        buy_cash_v = avail_v * BT_POSITION_PCT
        shares_v = int(buy_cash_v / (bar_close_v * 1.001) / TRADE_LOT_SIZE) * TRADE_LOT_SIZE
        if shares_v < TRADE_LOT_SIZE: shares_v = TRADE_LOT_SIZE
        _log('[回测] 初始买入 {}股 @{:.2f}'.format(shares_v, bar_close_v))
        try: order_shares(STOCK_QMT, shares_v, 'COMPETE', 0, ContextInfo, ACCOUNT)
        except Exception as e: _log('[回测] 下单失败: {}'.format(e))
    ...
    elif short_lots < MIN_POSITION_LOTS: short_reason = '可用{}<{}手'.format(base_can_use, MIN_POSITION_LOTS)
    ...
    # ---- 回测: OHLC交易判定 (日级去重) ----
    if not is_live:
        # ★ v7.2: 用close日期做日级去重 (QMT回测handlebar按分钟bar触发)
        bar_date = ContextInfo.bar_date if hasattr(ContextInfo, 'bar_date') else ''
        if not bar_date:
            # fallback: 从context获取当前交易日
            try: bar_date = ContextInfo.get_trade_date()
            except Exception: bar_date = ''
        if bar_date and bar_date == st.get('_bt_trade_date', ''):
            return  # 同一交易日已处理过, 跳过
        st['_bt_trade_date'] = bar_date
...
def _bt_trade_from_ohlc(ContextInfo, signal, open_p, high_p, low_p, close_p,
                        do_short, do_long):
    """回测: 用日线OHLC判断交易, 每bar最多一笔。下单用COMPETE避免分钟bar价格越界。"""
    st = ContextInfo.st
    ...
    if do_short:
        ...
        if high_p >= sell_trig:
            ...
            # ★ v7.2: 回测用 COMPETE, 避免分钟bar FIX越界
            _bt_order(ContextInfo, -TRADE_LOT_SIZE, sell_price)
            if buy_price > sell_price:
                _bt_order(ContextInfo, TRADE_LOT_SIZE, close_p)  # 亏损强平
            else:
                _bt_order(ContextInfo, TRADE_LOT_SIZE, buy_price)
...
def _bt_order(ContextInfo, shares, price_hint):
    """回测下单: 用 COMPETE 对手价, price_hint 仅用于日志"""
    try:
        order_shares(STOCK_QMT, shares, 'COMPETE', 0, ContextInfo, ACCOUNT)
...
def _print_status(ContextInfo, curr_price, avail_cash, pos_value, total_val):
    ...
    if st['total_t_days'] > 0: _log('累计:{}笔 {:+,.0f}'.format(st['total_t_days'], st['total_pnl']))
...
def stop(ContextInfo):
    ...
    if st:
        _log(''); _log('=' * 55)
        _log('  {} v7.2 停止  累计{}笔 毛利{:+,.0f}'.format(
