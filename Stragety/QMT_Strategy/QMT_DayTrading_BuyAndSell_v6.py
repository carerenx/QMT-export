...
"""
================================================================================
 QMT 迷你做T策略 v7.4 — 双方向版 (修复佣金 & 日志日期)
================================================================================
 QMT运行时注入: get_trade_detail_data / order_shares / ContextInfo.*

 【v7.4 改动】
   ★ 佣金: 代码直接算 (回测 deal_callback 不触发 THIS_CLOSE)
   ★ 日志: 每笔交易打印交易日期, 不再依赖 get_bar_time()
   ★ 日志: 紧凑格式, 无多余换行
================================================================================
"""
...
# 每笔交易固定费率 (双向各收)
TRADE_FEE_RATE = COMMISSION + STAMP_TAX  # 0.00125 per side
...
def _trade_fee(price, shares):
    """估算双向交易费用: (佣金+佣金) + 卖出印花税"""
    # 买入: 佣金, 卖出: 佣金+印花税
    buy_comm = price * shares * COMMISSION
    sell_comm = price * shares * COMMISSION
    sell_tax = price * shares * STAMP_TAX
    return buy_comm + sell_comm + sell_tax
...
def init(ContextInfo):
    ...
    ContextInfo.st = {
        ...
        '_bt_inited': False, '_last_trade_date': '',
    }
...
def handlebar(ContextInfo):
    ...
    if not is_live and base_shares < TRADE_LOT_SIZE and not st.get('_bt_inited'):
        ...
        _log('[回测] 初始买入 {}股 @{:.2f} 资金{:,.0f}'.format(shares_0, bar_close_0, BT_INIT_CASH))
    ...
    if not is_live:
        ...
        except Exception: trade_date = ''
        if trade_date and trade_date == st.get('_last_trade_date', ''):
            return
    ...
    if not is_live:
        _bt_trade_from_ohlc(ContextInfo, signal, bar_open, bar_high, bar_low, bar_close,
                            do_short, do_long, st.get('_last_trade_date', ''))
        return
...
def _bt_trade_from_ohlc(ContextInfo, signal, open_p, high_p, low_p, close_p,
                        do_short, do_long, trade_date=''):
    """用日线OHLC判断当日是否可成交。每天最多一笔。佣金代码直接算。"""
    st = ContextInfo.st
    atr_pct = signal['atr_pct']; traded = False
    dt = trade_date if trade_date else ''
    ...
    if do_short:
        ...
        if high_p >= sell_trig:
            ...
            fee = _trade_fee(sell_price, TRADE_LOT_SIZE) + _trade_fee(buy_price, TRADE_LOT_SIZE)
            net = gross - fee

            _log('[{}] 反T S{:.2f} B{:.2f} gross{:+,.0f} fee{:.0f} net{:+,.0f} | O{:.2f} H{:.2f} L{:.2f} C{:.2f}'.format(
                dt, sell_price, buy_price, gross, fee, net, open_p, high_p, low_p, close_p))
    ...
    if do_long and not traded:
        ...
        if low_p <= buy_trig:
            ...
            gross = (sell_price - buy_price) * TRADE_LOT_SIZE
            fee = _trade_fee(buy_price, TRADE_LOT_SIZE) + _trade_fee(sell_price, TRADE_LOT_SIZE)
            net = gross - fee

            _log('[{}] 正T B{:.2f} S{:.2f} gross{:+,.0f} fee{:.0f} net{:+,.0f} | O{:.2f} H{:.2f} L{:.2f} C{:.2f}'.format(
                dt, buy_price, sell_price, gross, fee, net, open_p, high_p, low_p, close_p))
...
def deal_callback(ContextInfo, deal):
    """成交回调: 仅实盘累加手续费 (回测THIS_CLOSE不触发此回调, 由代码直接算)"""
    st = ContextInfo.st
...
def _print_status(ContextInfo, curr_price, avail_cash, pos_value, total_val):
    ...
    if st['total_t_days'] > 0: _log('累计:{}笔 gross{:+,.0f} fee{:.0f} net{:+,.0f} W/L:{}/{}'.format(
        st['total_t_days'], st['total_pnl'], st.get('total_commission', 0),
        st['total_pnl'] - st.get('total_commission', 0),
        st.get('total_wins', 0), st.get('total_losses', 0)))
...
def stop(ContextInfo):
    ...
    if st and st.get('total_t_days', 0) > 0:
        ...
        _log('=' * 70)
        _log('  {} v7.4  {}d  gross{:+,.0f}  fee{:.0f}  net{:+,.0f}  W{}/L{}  winrate{:.0f}%'.format(
