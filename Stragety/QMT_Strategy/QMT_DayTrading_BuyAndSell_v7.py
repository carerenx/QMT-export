...
"""
================================================================================
 QMT 迷你做T策略 v7.5 — 双方向优化版
================================================================================
 QMT运行时注入: get_trade_detail_data / order_shares / ContextInfo.*

 【v7.5 vs v7.4 策略优化】
   ★ 最低净利过滤: 预期净利 < MIN_PROFIT(¥150) 的交易跳过, 避免佣金吃掉薄利
   ★ 动态手数: 随股价升高自动减手 (高价股 AT 值大但本金占用多, 控制风险)
   ★ 正T买入价更保守: 取 open/low/close 三者最低, 不硬取 buy_trigger(可能买在当天相对高位)
   ★ 反T卖出价更保守: 高波动日(ATR>8%)降低卖出触发乘数, 早卖早落袋
   ★ 回测去重: 用bar_close序列判重(替代 get_trade_date 返回空的问题)
   ★ 费率: 代码直接算双向合计每笔
================================================================================
"""
...
# ---- ★ v7.5 新增: 交易过滤 ----
MIN_PROFIT         = 150    # 最低净利(元), 低于此值的预期交易跳过
HIGH_ATR_THRESHOLD = 0.08   # ATR>8%视为高波动, 反T提前卖出
SELL_MULT_HI_ATR   = 0.30   # 高ATR时反T乘数降低
...
TRADE_FEE_RATE = COMMISSION + STAMP_TAX
...
def _dynamic_lots(price, base_lots=3):
    """★ v7.5: 动态手数 — 高价减少手数控制风险敞口。
    ¥100 → 3手, ¥200 → 2手, ¥300+ → 1手
    """
    if price >= 300: return 1
    elif price >= 200: return 2
    else: return base_lots
...
def compute_signal(opens, highs, lows, closes, volumes):
    ...
    # ★ v7.5: 高波动日反T乘数额外降低
    if curr_atr_pct > HIGH_ATR_THRESHOLD:
        sell_mult = min(sell_mult, SELL_MULT_HI_ATR)
...
def init(ContextInfo):
    ...
    ContextInfo.st = {
        ...
        'total_commission': 0.0, 'total_wins': 0, 'total_losses': 0,
        'total_skipped': 0,  # 因预期利润过低跳过的交易
        ...
        '_bt_inited': False,
        '_last_bar_date': '',      # 日级去重: 用日线的最后一个bar日期
    }
...
def handlebar(ContextInfo):
    ...
    if not is_live and base_shares < TRADE_LOT_SIZE and not st.get('_bt_inited'):
        ...
        shares_0 = int(buy_cash / (bar_close * 1.001) / TRADE_LOT_SIZE) * TRADE_LOT_SIZE
        ...
        st['_bt_inited'] = True
        _log('[回测] 初始买入 {}股 @{:.2f} 总资金{:,.0f}'.format(shares_0, bar_close, BT_INIT_CASH))
    ...
    bar_low = lows[STOCK_QMT][-1]

    # ----- ★ v7.5: 日级去重用 last bar close 作唯一日期 ----
    if not is_live:
        bar_date = str(round(bar_close, 2))  # 用收盘价作去重key (同一天所有分钟bar收盘价一样)
        if bar_date == st.get('_last_bar_date', ''):
            return
        st['_last_bar_date'] = bar_date
    ...
    # ★ v7.5: 动态手数
    dyn_lots = _dynamic_lots(bar_close)
    long_lots = min(long_lots, dyn_lots)
    short_lots = min(short_lots, dyn_lots)
    ...
    if not do_long:
        ...
        if dyn_lots < MIN_POSITION_LOTS: r.append('高价限手')
    ...
    st['pos_value'] = base_shares * curr_price
    st['pos_pct'] = st['pos_value'] / (st['pos_value'] + avail_cash) * 100 if avail_cash >= 0 else 0
    ...
    pos_value = st['pos_value']; total_val = pos_value + avail_cash
    _print_status(ContextInfo, curr_price, avail_cash, pos_value, total_val)
    _print_signal(ContextInfo)


# ============================================================================
# 第六部分：回测 OHLC 交易判定 (★ v7.5 优化)
# ============================================================================

def _bt_trade_from_ohlc(ContextInfo, signal, open_p, high_p, low_p, close_p,
                        do_short, do_long, short_lots, long_lots):
    """用日线OHLC判断当日是否可成交。每天最多一笔。"""
    st = ContextInfo.st
    atr_pct = signal['atr_pct']; traded = False

    # --- 反T ---
    if do_short and short_lots > 0:
        sell_trig = signal['sell_trigger']
        if high_p >= sell_trig:
            # ★ v7.5: 高波动日降低卖出触发, 早卖早落袋
            if atr_pct > HIGH_ATR_THRESHOLD:
                sell_trig = min(sell_trig, round(open_p * (1.0 + atr_pct * SELL_MULT_HI_ATR), 2))
            sell_price = max(open_p, sell_trig, close_p)  # 至少不低于三者最高
            ...
            gross = (sell_price - buy_price) * TRADE_LOT_SIZE * short_lots
            fee = _trade_fee(sell_price, TRADE_LOT_SIZE * short_lots) + _trade_fee(buy_price, TRADE_LOT_SIZE * short_lots)
            net = gross - fee

            # ★ v7.5: 预期净利不足 → 跳过
            if net < MIN_PROFIT and close_p > sell_trig * 0.99:
                st['total_skipped'] = st.get('total_skipped', 0) + 1
                return  # 不做这笔

            _log('S{:.2f}B{:.2f} {:+,.0f}f{:.0f}n{:+,.0f}|l{}|O{:.2f}H{:.2f}L{:.2f}C{:.2f}'.format(
                sell_price, buy_price, gross, fee, net, short_lots, open_p, high_p, low_p, close_p))

            for _ in range(short_lots):
            ...
            if net > 0: st['total_wins'] = st.get('total_wins', 0) + 1
            else: st['total_losses'] = st.get('total_losses', 0) + 1
            traded = True

    # --- 正T ---
    if do_long and long_lots > 0 and not traded:
        buy_trig = signal['buy_trigger']
        if low_p <= buy_trig:
            # ★ v7.5: 买入价取触发价、open、low、close的最低 → 更接近实际成交
            buy_price = min(buy_trig, open_p, close_p)
            if buy_price < low_p: buy_price = low_p  # 不低于当日最低价
            ...
            gross = (sell_price - buy_price) * TRADE_LOT_SIZE * long_lots
            fee = _trade_fee(buy_price, TRADE_LOT_SIZE * long_lots) + _trade_fee(sell_price, TRADE_LOT_SIZE * long_lots)
            net = gross - fee

            if net < MIN_PROFIT and close_p > buy_trig:
                st['total_skipped'] = st.get('total_skipped', 0) + 1
                return

            _log('B{:.2f}S{:.2f} {:+,.0f}f{:.0f}n{:+,.0f}|l{}|O{:.2f}H{:.2f}L{:.2f}C{:.2f}'.format(
                buy_price, sell_price, gross, fee, net, long_lots, open_p, high_p, low_p, close_p))

            for _ in range(long_lots):
                _bt_order(ContextInfo, TRADE_LOT_SIZE)
                _bt_order(ContextInfo, -TRADE_LOT_SIZE)
            st['total_t_days'] += 1; st['total_pnl'] += gross
            st['total_commission'] = st.get('total_commission', 0) + fee
...
def _bt_order(ContextInfo, shares):
    try: order_shares(STOCK_QMT, shares, 'THIS_CLOSE', 0, ContextInfo, ACCOUNT)
    except Exception: pass
...
def _print_status(ContextInfo, curr_price, avail_cash, pos_value, total_val):
    ...
    if st['total_t_days'] > 0:
        gross = st['total_pnl']; comm = st.get('total_commission', 0)
        _log('累计:{}笔 gross{:+,.0f} fee{:.0f} net{:+,.0f} W{} L{} skip{}'.format(
            st['total_t_days'], gross, comm, gross - comm,
            st.get('total_wins', 0), st.get('total_losses', 0), st.get('total_skipped', 0)))


def _print_signal(ContextInfo):
    st = ContextInfo.st; s = st['daily_signal']
    _log('Trend:{} O{:.2f} ATR{:.1f}% RSI{:.0f} Mult{:.2f} S:{} L:{}'.format(
...
def stop(ContextInfo):
    ...
    if st and st.get('total_t_days', 0) > 0:
        gross = st.get('total_pnl', 0); comm = st.get('total_commission', 0)
        w = st.get('total_wins', 0); l = st.get('total_losses', 0)
        s = st.get('total_skipped', 0)
        _log('=' * 70)
        _log('v7.5 {} {}d gross{:+,.0f} fee{:.0f} net{:+,.0f} W{}/L{} win{:.0f}% skip{}'.format(
            STOCK_NAME, st['total_t_days'], gross, comm, gross - comm, w, l,
            w/(w+l)*100 if (w+l) > 0 else 0, s))
        _log('=' * 70)
