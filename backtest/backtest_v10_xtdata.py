...
"""
================================================================================
 QMT 迷你反T v10 策略回测 — 基于 xtdata 本地数据
================================================================================
...
class BacktestEngine:
    ...
    def _simulate_day(self, signal, minute_bars, date, verbose):
        ...
        for idx, (bar_time, bar) in enumerate(minute_bars.iterrows()):
            ...
            # 止损检查 (仅在SOLD和DIPPING状态)
            if fstate in (STATE_SOLD, STATE_DIPPING) and not stop_loss_hit:
                # 用开盘价作为持仓市值基准
                loss_limit = TRADE_LOT_SIZE * open_price * STOP_LOSS_PCT
                # 估算当日PnL (简化: 卖价-当前价)*股数
    ...
    def save_csv(self, output_dir=None):
        ...
        # 交易记录
        trades_file = os.path.join(output_dir, f'v10_backtest_trades_{timestamp}.csv')
        trades_df = pd.DataFrame([t.to_dict() for t in self.results])
        trades_df.to_csv(trades_file, index=False, encoding='utf-8-sig')
        print(f'[保存] 交易记录: {trades_file}')

        # 每日PnL
        pnl_file = os.path.join(output_dir, f'v10_backtest_daily_{timestamp}.csv')
...
def main():
    parser = argparse.ArgumentParser(
        description='QMT 迷你反T v10 策略回测 (xtdata 本地数据)',
    )
