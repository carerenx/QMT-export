...
class BigQmtXtData:
    ...
    def connect(self):
        """MiniQMT compatibility no-op.

        The bridge client creates its transport lazily on the first RPC call,
        so there is no separate market-data connection to establish.
        """
        return 0

    def disconnect(self):
        """MiniQMT compatibility no-op; transports own their cleanup."""
        return 0
...
class BigQmtXtTrader:
    def __init__(
        ...
        # Newer MiniQMT builds accept the callback as the third positional
        # argument: XtQuantTrader(path, session_id, callback).  Earlier bridge
        # code used that slot as an optional account id, so retain string/int
        # account ids while recognizing callback-like objects.
        constructor_callback = None
        if account_id is not None and not isinstance(account_id, (str, int)):
            if any(
                callable(getattr(account_id, name, None))
                for name in ("on_stock_order", "on_stock_trade", "on_order_error")
            ):
                constructor_callback = account_id
                account_id = None
        self.path = path
        ...
        self.callback = constructor_callback
    ...
    def query_account_infos(self, account=None):
        rows = self._query_account_list(account, "query_account_infos")
        result = []
        for row in rows:
            if isinstance(row, dict):
                item = dict(row)
            else:
                item = dict(getattr(row, "__dict__", {}) or {})
            account_id = str(
                item.get("account_id")
                or item.get("m_strAccountID")
                or item.get("accountID")
                or item.get("id")
                or self.client.account_id
                or ""
            )
            account_type = item.get("account_type")
            if account_type is None:
                account_type = item.get("m_nAccountType", SECURITY_ACCOUNT)
            item.setdefault("account_id", account_id)
            item.setdefault("account_type", account_type)
            item.setdefault("m_strAccountID", account_id)
            item.setdefault("m_nAccountType", account_type)
            result.append(CompatObject(**item))
        return result
