...
class BigQmtStrategyRunnerTest(unittest.TestCase):
    ...
    def test_init_detects_bigqmt_account_from_runtime_global(self):
        ...
        try:
            # This case tests the fallback when no private server config exists.
            # A developer's configured account must not affect the assertion.
            with mock.patch.dict(sys.modules, {"bigqmt_signal_trader_local_config": None}):
                strategy_module.init(context)
        finally:
