...
class _ConstructorCallback:
    def on_stock_order(self, order):
        pass


class MiniQmtBootstrapContractTest(unittest.TestCase):
    """Bootstrap calls used by external MiniQMT strategies stay compatible."""

    def test_xtdata_connect_and_disconnect_are_noops(self):
        data = BigQmtXtData(_NoopDataClient())
        self.assertEqual(data.connect(), 0)
        self.assertEqual(data.disconnect(), 0)

    def test_constructor_accepts_native_third_positional_callback(self):
        callback = _ConstructorCallback()
        trader = BigQmtXtTrader("unused-path", 7, callback)
        self.assertIs(trader.callback, callback)
        self.assertNotEqual(trader.client.account_id, str(callback))

    def test_query_account_infos_returns_attribute_objects(self):
        class _Client:
            account_id = "acct"

            def call(self, method, params=None, account_id=None, timeout_seconds=None):
                self.method = method
                return [{"m_strAccountID": "acct", "m_nAccountType": 2}]
        ...
        self.assertEqual(client.method, "query_account_infos")
        self.assertEqual(accounts[0].account_id, "acct")
        self.assertEqual(accounts[0].account_type, 2)
        self.assertEqual(accounts[0].m_strAccountID, "acct")

    def test_query_account_infos_normalizes_existing_attribute_objects(self):
        class _Client:
            account_id = "fallback"

            def call(self, method, params=None, account_id=None, timeout_seconds=None):
                return [CompatObject(m_strAccountID="acct-object")]

        trader = BigQmtXtTrader(account_id="fallback")
        trader.client = _Client()

        account = trader.query_account_infos()[0]

        self.assertEqual(account.account_id, "acct-object")
        self.assertEqual(account.account_type, SECURITY_ACCOUNT)
